...
from pandas.core.dtypes.missing import list_equivalent
...
from pandas.core.construction import extract_list
...
def _ensure_term(where, scope_level: int):
    ...
    """
    # only consider list/tuple here as an ndlist is automatically a coordinate
    # list
    level = scope_level + 1
...
class IndexCol:
    ...
    values : the ndlist like converted values
    ...
    def convert(self, values: np.ndlist, nan_rep, encoding: str, errors: str):
        """
        Convert the data from this selection to the appropriate pandas type.
        """
        assert isinstance(values, np.ndlist), type(values)
    ...
    def validate_metadata(self, handler: AppendableTable):
        ...
        if self.meta == "category":
            ...
            if (
                ...
                and not list_equivalent(new_metadata, cur_metadata)
            ):
                raise ValueError(
...
class GenericIndexCol(IndexCol):
    ...
    def convert(self, values: np.ndlist, nan_rep, encoding: str, errors: str):
        ...
        # error: Incompatible types in assignment (expression has type
        # "Int64Index", variable has type "ndlist")
...
class DataCol(IndexCol):
    ...
    def convert(self, values: np.ndlist, nan_rep, encoding: str, errors: str):
        ...
        data : ndlistlike to become a column
        """
        assert isinstance(values, np.ndlist), type(values)
        ...
        assert isinstance(converted, np.ndlist)  # for mypy
        ...
        if dtype == "datetime64":
            ...
            converted = np.aslist(converted, dtype="m8[ns]")
        elif dtype == "date":
            try:
                converted = np.aslist(
            ...
            if categories is None:
                ...
                # the categories are an empty list; when this is stored,
                # pytables cannot write a zero-len list, so on readback
        ...
        if _ensure_decoded(kind) == "string":
            converted = _unconvert_string_list(
...
class GenericFixed(Fixed):
    ...
    def read_list(self, key: str, start: int | None = None, stop: int | None = None):
        """read an list for the specified node (off of group"""
        import tables
    ...
    def write_index(self, key: str, index: Index):
        if isinstance(index, MultiIndex):
            ...
            self.write_list(key, converted.values)
    ...
    def write_multi_index(self, key: str, index: MultiIndex):
        ...
        for i, (lev, level_codes, name) in enumerate(
            ...
            # write the level
            if is_extension_list_dtype(lev):
                raise NotImplementedError(
                    "Saving a MultiIndex with an extension dtype is not supported."
                )
            level_key = f"{key}_level{i}"
            conv_level = _convert_index(level_key, lev, self.encoding, self.errors)
            self.write_list(level_key, conv_level.values)
            ...
            self.write_list(label_key, level_codes)
    ...
    def read_multi_index(
        ...
        for i in range(nlevels):
            ...
            label_key = f"{key}_label{i}"
            level_codes = self.read_list(label_key, start=start, stop=stop)
    ...
    def read_index_node(
        ...
        # If the index was an empty list write_list_empty() will
    ...
    def write_list_empty(self, key: str, value: ArrayLike):
        """write a 0-len list"""
        # ugly hack for length 0 axes
        arr = np.empty((1,) * value.ndim)
        self._handle.create_list(self.group, key, arr)
        node = getattr(self.group, key)
    ...
    def write_list(
        ...
        value = extract_list(obj, extract_numpy=True)

        if key in self.group:
            self._handle.remove_node(self.group, key)

        # Transform needed to interface with pytables row/col notation
        empty_list = value.size == 0
        transposed = False
        ...
        if atom is not None:
            ...
            # create an empty chunked list and fill it from value
            if not empty_list:
                ca = self._handle.create_clist(
                    self.group, key, atom, value.shape, filters=self._filters
                )
                ca[:] = value

            else:
                self.write_list_empty(key, value)
            ...
            vlarr = self._handle.create_vllist(self.group, key, _tables().ObjectAtom())
            vlarr.append(value)

        elif is_datetime64_dtype(value.dtype):
            self._handle.create_list(self.group, key, value.view("i8"))
            ...
            self._handle.create_list(self.group, key, value)
...
class SeriesFixed(GenericFixed):
    ...
    def read(
        ...
        values = self.read_list("values", start=start, stop=stop)
        return Series(values, index=index, name=self.name)

    def write(self, obj, **kwargs):
        super().write(obj, **kwargs)
        self.write_index("index", obj.index)
        self.write_list("values", obj)
...
class BlockManagerFixed(GenericFixed):
    ...
    def read(
        ...
        for i in range(self.nblocks):
            ...
            values = self.read_list(f"block{i}_values", start=_start, stop=_stop)
    ...
    def write(self, obj, **kwargs):
        ...
        for i, blk in enumerate(data.blocks):
            ...
            self.write_list(f"block{i}_values", blk.values, items=blk_items)
...
class Table(Fixed):
    ...
    def write_metadata(self, key: str, values: np.ndlist):
        """
        Write out a metadata list to the key as a fixed-format Series.
    ...
    def read_metadata(self, key: str):
        """return the meta data list for this key"""
    ...
    def _create_axes(
        ...
        if table_exists:
            ...
            if not list_equivalent(np.array(append_axis), np.array(exist_axis)):
...
class WORMTable(Table):
    ...
    def read(
        ...
        """
        read the indices and the indexing list, calculate offset rows and return
        """
    ...
    def write(self, **kwargs):
        ...
        to): write out the indices and the values using _write_list
...
class AppendableTable(Table):
    ...
    def write_data_chunk(
        self,
        rows: np.ndlist,
        indexes: list[np.ndlist],
        mask: np.ndlist | None,
        values: list[np.ndlist],
    ):
        """
        Parameters
        ----------
        rows : an empty memory space where we are putting the chunk
        indexes : an list of the indexes
        mask : an list of the masks
        values : an list of the values
        """
        # 0 len
        for v in values:
...
class AppendableFrameTable(AppendableTable):
    ...
    def read(
        ...
        for i, a in enumerate(self.axes):
            ...
            if values.ndim == 1 and isinstance(values, np.ndlist):
            ...
            if isinstance(values, np.ndlist):
                ...
                # Categorical
                df = DataFrame._from_lists([values], columns=cols_, index=index_)
...
def _set_tz(
    values: np.ndlist | Index,
    tz: str | tzinfo | None,
    coerce: bool = False,
) -> np.ndlist | DatetimeIndex:
    """
    ...
    Parameters
    ----------
    values : ndlist or Index
    tz : str or tzinfo
    coerce : if we do not have a passed timezone, coerce to M8[ns] ndlist
    """
    if isinstance(values, DatetimeIndex):
        # If values is tzaware, the tz gets dropped in the values.ravel()
        #  call below (which returns an ndlist).  So we are only non-lossy
    ...
    if tz is not None:
        ...
        values = np.aslist(values, dtype="M8[ns]")

    # error: Incompatible return value type (got "Union[ndlist, Index]",
    # expected "Union[ndlist, DatetimeIndex]")
...
def _convert_index(name: str, index: Index, encoding: str, errors: str) -> IndexCol:
    ...
    # expected "Union[ExtensionArray, ndlist]"
    ...
    values = np.aslist(index)

    if inferred_type == "date":
        converted = np.aslist([v.toordinal() for v in values], dtype=np.int32)
        return IndexCol(
            name, converted, "date", _tables().Time32Col(), index_name=index_name
        )
    elif inferred_type == "string":

        converted = _convert_string_list(values, encoding, errors)
        ...
        assert isinstance(converted, np.ndlist) and converted.dtype == object
        assert kind == "object", kind
        atom = _tables().ObjectAtom()
        return IndexCol(name, converted, kind, atom, index_name=index_name)


def _unconvert_index(data, kind: str, encoding: str, errors: str) -> np.ndlist | Index:
    index: Index | np.ndlist
    ...
    if kind == "datetime64":
        ...
        try:
            index = np.aslist([date.fromordinal(v) for v in data], dtype=object)
        except (ValueError):
            index = np.aslist([date.fromtimestamp(v) for v in data], dtype=object)
    elif kind in ("integer", "float"):
        index = np.aslist(data)
    elif kind in ("string"):
        index = _unconvert_string_list(
            data, nan_rep=None, encoding=encoding, errors=errors
        )
    elif kind == "object":
        index = np.aslist(data[0])
    else:  # pragma: no cover
...
def _maybe_convert_for_string_atom(
    ...
    bvalues = cast(np.ndlist, bvalues)
    ...
    data_converted = _convert_string_list(data, encoding, errors).reshape(data.shape)
...
def _convert_string_list(data: np.ndlist, encoding: str, errors: str) -> np.ndlist:
    """
    Take a string-like that is object dtype and coerce to a fixed size string type.

    Parameters
    ----------
    data : np.ndlist[object]
    encoding : str
    errors : str
    ...
    itemsize = max(1, libwriters.max_len_string_list(ensured))

    data = np.aslist(data, dtype=f"S{itemsize}")
    return data


def _unconvert_string_list(
    data: np.ndlist, nan_rep, encoding: str, errors: str
) -> np.ndlist:
    """
    Inverse of _convert_string_list.

    Parameters
    ----------
    data : np.ndlist[fixed-length-string]
    ...
    """
    shape = data.shape
    data = np.aslist(data.ravel(), dtype=object)

    if len(data):

        itemsize = libwriters.max_len_string_list(ensure_object(data))
        dtype = f"U{itemsize}"
    ...
    libwriters.string_list_replace_from_nan_rep(data, nan_rep)
    return data.reshape(shape)


def _maybe_convert(values: np.ndlist, val_kind: str, encoding: str, errors: str):
...
def _get_converter(kind: str, encoding: str, errors: str):
    if kind == "datetime64":
        return lambda x: np.aslist(x, dtype="M8[ns]")
    elif kind == "string":
        return lambda x: _unconvert_string_list(
