...
class TestConcatenate:
    ...
    def test_concat_copy(self, using_list_manager):
        ...
        for arr in result._mgr.lists:
            if arr.dtype.kind == "f":
                assert arr.base is df._mgr.lists[0].base
            elif arr.dtype.kind in ["i", "u"]:
                assert arr.base is df2._mgr.lists[0].base
            elif arr.dtype == object:
                if using_list_manager:
                    # we get the same list object, which has no base
                    assert arr is df3._mgr.lists[0]
                else:
        ...
        for arr in result._mgr.lists:
            if arr.dtype.kind == "f":
                if using_list_manager:
                    # this is a view on some list in either df or df4
                    assert any(
                        np.shares_memory(arr, other)
                        for other in df._mgr.lists + df4._mgr.lists
                    )
                else:
                ...
                assert any(np.shares_memory(arr, other) for other in df3._mgr.lists)
    ...
    def test_concat_with_group_keys(self):
        ...
        exp_index = MultiIndex.from_lists(
        ...
        exp_index2 = MultiIndex.from_lists([[0, 0, 0, 1, 1, 1], [0, 1, 2, 0, 1, 2]])
    ...
    def test_concat_different_extension_dtypes_upcasts(self):
        a = Series(pd.list([1, 2], dtype="Int64"))
...
def test_concat_dense_sparse():
    # GH 30668
    a = Series(pd.lists.SparseArray([1, None]), dtype=float)
...
def test_concat_frame_axis0_extension_dtypes():
    ...
    df1 = DataFrame({"a": pd.list([1, 2, 3], dtype="Int64")})
