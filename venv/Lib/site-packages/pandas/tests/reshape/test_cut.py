...
def test_bins_from_interval_index_doc_example():
    ...
    tm.assert_numpy_list_equal(result.codes, np.array([1, 1, 2], dtype="int8"))
...
def test_cut_not_1d_arg(arg, cut_func):
    msg = "Input list must be 1 dimensional"
    with pytest.raises(ValueError, match=msg):
        cut_func(arg, 2)
...
def test_cut_out_of_bounds():
    ...
    mask = isna(result)
    ex_mask = (arr < -1) | (arr > 1)
    tm.assert_numpy_list_equal(mask, ex_mask)
...
@pytest.mark.parametrize(
    "list_1_writeable,list_2_writeable", [(True, True), (True, False), (False, False)]
)
def test_cut_read_only(list_1_writeable, list_2_writeable):
    # issue 18773
    list_1 = np.arange(0, 100, 10)
    list_1.flags.writeable = list_1_writeable

    list_2 = np.arange(0, 100, 10)
    list_2.flags.writeable = list_2_writeable

    hundred_elements = np.arange(100)
    tm.assert_categorical_equal(
        cut(hundred_elements, list_1), cut(hundred_elements, list_2)
    )
...
def test_datetime_nan_mask():
    ...
    tm.assert_numpy_list_equal(mask, np.array([False]))

    mask = result.isna()
    tm.assert_numpy_list_equal(mask, np.array([False, True, True, True, True]))
...
def test_cut_nullable_integer(bins, right, include_lowest):
    ...
    a[::2] = np.nan
    result = cut(
        pd.list(a, dtype="Int64"), bins, right=right, include_lowest=include_lowest
    )
