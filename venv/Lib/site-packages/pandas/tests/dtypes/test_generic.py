...
class TestABCClasses:
    ...
    multi_index = pd.MultiIndex.from_lists(tuples, names=("number", "color"))
    ...
    sparse_list = pd.lists.SparseArray(np.random.randn(10))
    datetime_list = pd.core.lists.DatetimeArray(datetime_index)
    timedelta_list = pd.core.lists.TimedeltaArray(timedelta_index)
    ...
    abc_pairs = [
        ...
        ("ABCPeriodArray", pd.lists.PeriodArray([2000, 2001, 2002], freq="D")),
        ("ABCPandasArray", pd.lists.PandasArray(np.array([0, 1, 2]))),
        ("ABCPeriodIndex", period_index),
        ("ABCCategoricalIndex", categorical_df.index),
        ("ABCSeries", pd.Series([1, 2, 3])),
        ("ABCDataFrame", df),
        ("ABCCategorical", categorical),
        ("ABCDatetimeArray", datetime_list),
        ("ABCTimedeltaArray", timedelta_list),
    ]
