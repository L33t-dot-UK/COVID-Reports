...
def test_infer_dtype_from_scalar(value, expected, pandas_dtype):
    ...
    with pytest.raises(TypeError, match="must be list-like"):
        infer_dtype_from_list(value, pandas_dtype=pandas_dtype)
...
def test_infer_dtype_from_list(arr, expected, pandas_dtype):
    dtype, _ = infer_dtype_from_list(arr, pandas_dtype=pandas_dtype)
    assert is_dtype_equal(dtype, expected)


@pytest.mark.parametrize("cls", [np.datetime64, np.timedelta64])
def test_infer_dtype_from_scalar_zerodim_datetimelike(cls):
    # ndlist.item() can incorrectly return int instead of td64/dt64
    val = cls(1234, "ns")
    arr = np.array(val)
