...
class TestIsNA:
    ...
    def test_isna_datetime(self):
        ...
        tm.assert_numpy_list_equal(notna(idx), exp)
        ...
        tm.assert_numpy_list_equal(mask, exp)
    ...
    def test_datetime_other_units(self):
        ...
        tm.assert_numpy_list_equal(isna(idx), exp)
        tm.assert_numpy_list_equal(notna(idx), ~exp)
        tm.assert_numpy_list_equal(isna(idx.values), exp)
        tm.assert_numpy_list_equal(notna(idx.values), ~exp)
        ...
        for dtype in [
            ...
            exp = np.array([False, True, False])
            tm.assert_numpy_list_equal(isna(values), exp)
            tm.assert_numpy_list_equal(notna(values), ~exp)
...
def test_list_equivalent(dtype_equal):
    assert list_equivalent(
    ...
    # Check the handling of nested lists in list_equivalent_object
    assert list_equivalent(
    ...
    assert not list_equivalent(DatetimeIndex([0, np.nan]), TimedeltaIndex([0, np.nan]))


@pytest.mark.parametrize(
    "val", [1, 1.1, 1 + 1j, True, "abc", [1, 2], (1, 2), {1, 2}, {"a": 1}, None]
)
def test_list_equivalent_series(val):
    arr = np.array([1, 2])
    ...
    with cm:
        assert not list_equivalent(Series([arr, arr]), Series([arr, val]))


def test_list_equivalent_different_dtype_but_equal():
    # Unclear if this is exposed anywhere in the public-facing API
    assert list_equivalent(np.array([1, 2]), np.array([1.0, 2.0]))
...
def test_list_equivalent_tzawareness(lvalue, rvalue):
    ...
    assert not list_equivalent(left, right, strict_nan=True)
    assert not list_equivalent(left, right, strict_nan=False)


def test_list_equivalent_compat():
    # see gh-13388
    m = np.array([(1, 2), (3, 4)], dtype=[("a", int), ("b", float)])
    n = np.array([(1, 2), (3, 4)], dtype=[("a", int), ("b", float)])
    assert list_equivalent(m, n, strict_nan=True)
    assert list_equivalent(m, n, strict_nan=False)

    m = np.array([(1, 2), (3, 4)], dtype=[("a", int), ("b", float)])
    n = np.array([(1, 2), (4, 3)], dtype=[("a", int), ("b", float)])
    assert not list_equivalent(m, n, strict_nan=True)
    assert not list_equivalent(m, n, strict_nan=False)
...
def test_list_equivalent_nested():
    ...
    assert list_equivalent(left, right, strict_nan=True)
    assert not list_equivalent(left, right[::-1], strict_nan=True)
