...
class TestToDatetime:
    ...
    def test_to_datetime_list_of_dt64s(self, cache, unit):
        ...
        # an list that is equal to Timestamp() parsing
    ...
    def test_datetime_bool_lists_mixed(self, cache):
...
class TestToDatetimeUnit:
    ...
    def test_unit_list_mixed_nans(self, cache):
    ...
    def test_unit_list_mixed_nans_large_int(self, cache):
    ...
    def test_unit_mixed(self, cache, exp, arr):
        ...
        msg = "mixed datetimes and integers in passed list"
        with pytest.raises(ValueError, match=msg):
...
class TestToDatetimeMisc:
    ...
    def test_to_datetime_strings_variation(self, cache):
        list = ["2012", "20120101", "20120101 12:01:01"]
        expected = list(to_datetime(list, cache=cache))
        result = [Timestamp(date_str) for date_str in list]
    ...
    def test_string_na_nat_conversion(self, cache):
        ...
        result = tslib.list_to_datetime(strings)[0]
        tm.assert_almost_equal(result, expected)

        result2 = to_datetime(strings, cache=cache)
        assert isinstance(result2, DatetimeIndex)
        tm.assert_numpy_list_equal(result, result2.values)
...
class TestGuessDatetimeFormat:
    ...
    def test_guess_datetime_format_for_list(self, test_list):
        expected_format = "%Y-%m-%d %H:%M:%S.%f"
        assert tools._guess_datetime_format_for_list(test_list) == expected_format

    @td.skip_if_not_us_locale
    def test_guess_datetime_format_for_list_all_nans(self):
        format_for_string_of_nans = tools._guess_datetime_format_for_list(
