...
def test_parsing_valid_dates(data, expected):
    ...
    result, _ = tslib.list_to_datetime(arr)
...
def test_parsing_timezone_offsets(dt_string, expected_tz):
    ...
    expected, _ = tslib.list_to_datetime(arr)

    arr = np.array([dt_string], dtype=object)
    result, result_tz = tslib.list_to_datetime(arr)
...
def test_parsing_different_timezone_offsets():
    ...
    result, result_tz = tslib.list_to_datetime(data)
    expected = np.array(
        [
...
def test_number_looking_strings_not_into_datetime(data):
    ...
    arr = np.array(data, dtype=object)
    result, _ = tslib.list_to_datetime(arr, errors="ignore")
...
def test_coerce_outside_ns_bounds(invalid_date, errors):
    ...
    if errors == "raise":
        ...
        with pytest.raises(ValueError, match=msg):
            tslib.list_to_datetime(**kwargs)
    else:  # coerce.
        result, _ = tslib.list_to_datetime(**kwargs)
...
def test_coerce_outside_ns_bounds_one_valid():
    ...
    result, _ = tslib.list_to_datetime(arr, errors="coerce")
...
def test_coerce_of_invalid_datetimes(errors):
    ...
    if errors == "ignore":
        ...
        tm.assert_numpy_list_equal(result, np.array(expected, dtype="M8[ns]"))
