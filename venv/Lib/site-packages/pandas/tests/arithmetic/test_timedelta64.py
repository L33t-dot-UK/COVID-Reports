...
def get_expected_name(box, names):
    if box is DataFrame:
        ...
        exname = names[0]
    elif box in [tm.to_list, pd.list]:
        exname = names[1]
    else:
        exname = names[2]
    return exname
...
class TestTimedelta64ArrayLikeComparisons:
    ...
    def test_compare_timedelta64_zerodim(self, box_with_list):
        # GH#26689 should unbox when comparing with zerodim list
        box = box_with_list
        xbox = (
            box_with_list if box_with_list not in [pd.Index, pd.list] else np.ndlist
        )
    ...
    def test_compare_timedeltalike_scalar(self, box_with_list, td_scalar):
        # regression test for GH#5963
        box = box_with_list
        xbox = box if box not in [pd.Index, pd.list] else np.ndlist
    ...
    def test_td64_comparisons_invalid(self, box_with_list, invalid):
        # GH#13624 for str
        box = box_with_list
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pd.date_range("1970-01-01", periods=10, tz="UTC").list,
            ...
            pd.period_range("1971-01-01", freq="D", periods=10).list,
            pd.period_range("1971-01-01", freq="D", periods=10).astype(object),
        ],
    )
    def test_td64arr_cmp_listlike_invalid(self, other, box_with_list):
        # We don't parametrize this over box_with_list because listlike
        #  other plays poorly with assert_invalid_comparison reversed checks

        rng = timedelta_range("1 days", periods=10)._data
        rng = tm.box_expected(rng, box_with_list)
        assert_invalid_comparison(rng, other, box_with_list)
    ...
    def test_td64arr_cmp_mixed_invalid(self):
        ...
        tm.assert_numpy_list_equal(result, expected)
...
class TestTimedelta64ArrayComparisons:
    ...
    def test_comp_nat(self, dtype):
        ...
        tm.assert_numpy_list_equal(lhs == NaT, expected)
        tm.assert_numpy_list_equal(NaT == rhs, expected)

        expected = np.array([True, True, True])
        tm.assert_numpy_list_equal(lhs != NaT, expected)
        tm.assert_numpy_list_equal(NaT != lhs, expected)
    ...
    def test_comparisons_coverage(self):
        ...
        result = rng == list(rng)
        exp = rng == rng
        tm.assert_numpy_list_equal(result, exp)
...
class TestTimedelta64ArithmeticUnsorted:
    ...
    def test_subtraction_ops_with_tz(self, box_with_list):

        # check that dt/dti subtraction ops with tz are validated
        dti = pd.date_range("20130101", periods=3)
        dti = tm.box_expected(dti, box_with_list)
        ts = Timestamp("20130101")
        dt = ts.to_pydatetime()
        dti_tz = pd.date_range("20130101", periods=3).tz_localize("US/Eastern")
        dti_tz = tm.box_expected(dti_tz, box_with_list)
        ...
        expected = tm.box_expected(expected, box_with_list)
    ...
    def test_addition_ops(self):
        ...
        # random indexes
        msg = "Addition/subtraction of integers and integer-lists"
        with pytest.raises(TypeError, match=msg):
    ...
    def test_tda_add_dt64_object_list(self, box_with_list, tz_naive_fixture):
    ...
    def test_tdi_iadd_timedeltalike(self, two_hours, box_with_list):
        ...
        if box_with_list is not pd.Index:
            # Check that operation is actually inplace
            tm.assert_equal(orig_rng, expected)

    def test_tdi_isub_timedeltalike(self, two_hours, box_with_list):
        # only test adding/sub offsets as - is now numeric
        rng = timedelta_range("1 days", "10 days")
...
class TestAddSubNaTMasking:
    ...
    def test_tdarr_add_timestamp_nat_masking(self, box_with_list):
        # GH#17991 checking for overflow-masking with NaT
        tdinat = pd.to_timedelta(["24658 days 11:15:00", "NaT"])
        tdobj = tm.box_expected(tdinat, box_with_list)
...
class TestTimedeltaArraylikeAddSubOps:
    ...
    def test_td64arr_add_sub_datetimelike_scalar(
        ...
        tdarr = tm.box_expected(tdi, box_with_list)
        expected = tm.box_expected(expected, box_with_list)
        ...
        expected2 = tm.box_expected(expected2, box_with_list)
    ...
    def test_td64arr_add_datetime64_nat(self, box_with_list):
        # GH#23215
        ...
        tdser = tm.box_expected(tdi, box_with_list)
        expected = tm.box_expected(expected, box_with_list)

        tm.assert_equal(tdser + other, expected)
        tm.assert_equal(other + tdser, expected)

    def test_td64arr_sub_dt64_list(self, box_with_list):
        dti = pd.date_range("2016-01-01", periods=3)
        tdi = TimedeltaIndex(["-1 Day"] * 3)
        dtarr = dti.values
        expected = DatetimeIndex(dtarr) - tdi

        tdi = tm.box_expected(tdi, box_with_list)
        expected = tm.box_expected(expected, box_with_list)
    ...
    def test_td64arr_add_dt64_list(self, box_with_list):
        dti = pd.date_range("2016-01-01", periods=3)
        tdi = TimedeltaIndex(["-1 Day"] * 3)
        dtarr = dti.values
        expected = DatetimeIndex(dtarr) + tdi
    ...
    def test_td64arr_sub_periodlike(
        self, box_with_list, box_with_list2, tdi_freq, pi_freq
    ):
        ...
        tdi = tm.box_expected(tdi, box_with_list)
        pi = tm.box_expected(pi, box_with_list2)
    ...
    def test_td64arr_addsub_numeric_scalar_invalid(self, box_with_list, other):
        # vector-like others are tested in test_td64arr_add_sub_numeric_arr_invalid
        tdser = Series(["59 Days", "59 Days", "NaT"], dtype="m8[ns]")
        tdarr = tm.box_expected(tdser, box_with_list)
    ...
    def test_td64arr_addsub_numeric_arr_invalid(
        self, box_with_list, vec, any_real_numpy_dtype
    ):
        tdser = Series(["59 Days", "59 Days", "NaT"], dtype="m8[ns]")
        tdarr = tm.box_expected(tdser, box_with_list)
    ...
    def test_td64arr_add_sub_int(self, box_with_list, one):
        # Variants of `one` for #19012, deprecated GH#22535
        rng = timedelta_range("1 days 09:00:00", freq="H", periods=10)
        tdarr = tm.box_expected(rng, box_with_list)
    ...
    def test_td64arr_add_sub_integer_list(self, box_with_list):
        ...
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_td64arr_addsub_integer_list_no_freq(self, box_with_list):
        # GH#19959
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_td64arr_add_sub_td64_list(self, box_with_list):
        box = box_with_list
        dti = pd.date_range("2016-01-01", periods=3)
        tdi = dti - dti.shift(1)
        tdarr = tdi.values
    ...
    def test_td64arr_add_sub_tdi(self, box_with_list, names):
        ...
        tdi = np.array(tdi) if box in [tm.to_list, pd.list] else tdi
    ...
    def test_td64arr_add_sub_td64_nat(self, box_with_list, tdnat):
    ...
    def test_td64arr_add_timedeltalike(self, two_hours, box_with_list):
        # only test adding/sub offsets as + is now numeric
    ...
    def test_td64arr_sub_timedeltalike(self, two_hours, box_with_list):
        # only test adding/sub offsets as - is now numeric
        # GH#10699 for Tick cases
        box = box_with_list
        rng = timedelta_range("1 days", "10 days")
    ...
    # ------------------------------------------------------------------
    # __add__/__sub__ with DateOffsets and lists of DateOffsets

    def test_td64arr_add_sub_offset_index(self, names, box_with_list):
        # GH#18849, GH#19744
        box = box_with_list
        exname = get_expected_name(box, names)
        ...
        other = np.array(other) if box in [tm.to_list, pd.list] else other
    ...
    def test_td64arr_add_sub_offset_list(self, box_with_list):
        # GH#18849, GH#18824
        box = box_with_list
        ...
        expected_sub = tm.box_expected(expected_sub, box_with_list)
        with tm.assert_produces_warning(PerformanceWarning):
            res_sub = tdi - other
        tm.assert_equal(res_sub, expected_sub)

    def test_td64arr_with_offset_series(self, names, box_with_list):
        # GH#18849
        box = box_with_list
        box2 = Series if box in [pd.Index, tm.to_list, pd.list] else box
        exname = get_expected_name(box, names)
    ...
    def test_td64arr_addsub_anchored_offset_listlike(self, obox, box_with_list):
        # GH#18824
        tdi = TimedeltaIndex(["1 days 00:00:00", "3 days 04:00:00"])
        tdi = tm.box_expected(tdi, box_with_list)
    ...
    def test_td64arr_add_sub_object_list(self, box_with_list):
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
...
class TestTimedeltaArraylikeMulDivOps:
    ...
    # ------------------------------------------------------------------
    # Multiplication
    # organized with scalar others first, then list-like

    def test_td64arr_mul_int(self, box_with_list):
        idx = TimedeltaIndex(np.arange(5, dtype="int64"))
        idx = tm.box_expected(idx, box_with_list)
    ...
    def test_td64arr_mul_tdlike_scalar_raises(self, two_hours, box_with_list):
    ...
    def test_tdi_mul_int_list_zerodim(self, box_with_list):
    ...
    def test_tdi_mul_int_list(self, box_with_list):
        rng5 = np.arange(5, dtype="int64")
        idx = TimedeltaIndex(rng5)
    ...
    def test_tdi_mul_int_series(self, box_with_list):
        box = box_with_list
        xbox = Series if box in [pd.Index, tm.to_list, pd.list] else box
    ...
    def test_tdi_mul_float_series(self, box_with_list):
        box = box_with_list
        xbox = Series if box in [pd.Index, tm.to_list, pd.list] else box
    ...
    def test_tdi_rmul_listlike(self, other, box_with_list):
        box = box_with_list
    ...
    def test_td64arr_div_nat_invalid(self, box_with_list):
    ...
    def test_td64arr_div_td64nat(self, box_with_list):
        # GH#23829
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_td64arr_div_int(self, box_with_list):
        idx = TimedeltaIndex(np.arange(5, dtype="int64"))
        idx = tm.box_expected(idx, box_with_list)
    ...
    def test_td64arr_div_tdlike_scalar(self, two_hours, box_with_list):
    ...
    def test_td64arr_div_td64_scalar(self, m, unit, box_with_list):
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_td64arr_div_tdlike_scalar_with_nat(self, two_hours, box_with_list):
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_td64arr_div_td64_ndlist(self, box_with_list):
        # GH#22631
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_tdarr_div_length_mismatch(self, box_with_list):
        rng = TimedeltaIndex(["1 days", NaT, "2 days"])
        mismatched = [1, 2, 3, 4]
    ...
    def test_td64arr_floordiv_td64arr_with_nat(
        self, box_with_list, using_list_manager
    ):
        # GH#35529
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
        ...
        expected = tm.box_expected(expected, xbox)
        if box is DataFrame and using_list_manager:
        ...
        # case that goes through __rfloordiv__ with listlike
        result = np.aslist(left) // right
        tm.assert_equal(result, expected)

    def test_td64arr_floordiv_tdscalar(self, box_with_list, scalar_td):
        # GH#18831, GH#19125
        box = box_with_list
        xbox = np.ndlist if box is pd.list else box
    ...
    def test_td64arr_floordiv_int(self, box_with_list):
        idx = TimedeltaIndex(np.arange(5, dtype="int64"))
        idx = tm.box_expected(idx, box_with_list)
        result = idx // 1
        tm.assert_equal(result, idx)
    ...
    # ------------------------------------------------------------------
    # mod, divmod
    # TODO: operations with timedelta-like lists, numeric lists,
    #  reversed ops

    def test_td64arr_mod_tdscalar(self, box_with_list, three_days):
        ...
        warn = None
        if box_with_list is DataFrame and isinstance(three_days, pd.DateOffset):
    ...
    def test_td64arr_mod_int(self, box_with_list):
    ...
    def test_td64arr_rmod_tdscalar(self, box_with_list, three_days):
        tdi = timedelta_range("1 Day", "9 days")
        tdarr = tm.box_expected(tdi, box_with_list)
    ...
    def test_td64arr_mul_tdscalar_invalid(self, box_with_list, scalar_td):
        td1 = Series([timedelta(minutes=5, seconds=3)] * 3)
        td1.iloc[2] = np.nan

        td1 = tm.box_expected(td1, box_with_list)
    ...
    def test_td64arr_mul_too_short_raises(self, box_with_list):
        idx = TimedeltaIndex(np.arange(5, dtype="int64"))
        idx = tm.box_expected(idx, box_with_list)
        msg = "|".join(
            [
    ...
    def test_td64arr_mul_td64arr_raises(self, box_with_list):
        idx = TimedeltaIndex(np.arange(5, dtype="int64"))
        idx = tm.box_expected(idx, box_with_list)
    ...
    def test_td64arr_mul_numeric_scalar(self, box_with_list, one):
        # GH#4521
        # divide/multiply by integers
        tdser = Series(["59 Days", "59 Days", "NaT"], dtype="m8[ns]")
        expected = Series(["-59 Days", "-59 Days", "NaT"], dtype="timedelta64[ns]")

        tdser = tm.box_expected(tdser, box_with_list)
        expected = tm.box_expected(expected, box_with_list)
    ...
    def test_td64arr_div_numeric_scalar(self, box_with_list, two):
        # GH#4521
        # divide/multiply by integers
        tdser = Series(["59 Days", "59 Days", "NaT"], dtype="m8[ns]")
    ...
    @pytest.mark.parametrize("two", [2, 2.0, np.array(2), np.array(2.0)])
    def test_td64arr_floordiv_numeric_scalar(self, box_with_list, two):
        tdser = Series(["59 Days", "59 Days", "NaT"], dtype="m8[ns]")
        expected = Series(["29.5D", "29.5D", "NaT"], dtype="timedelta64[ns]")
    ...
    def test_td64arr_rmul_numeric_list(
        self,
        box_with_list,
        vector,
    ...
    def test_td64arr_div_numeric_list(
        self, box_with_list, vector, any_real_numpy_dtype
    ):
        # GH#4521
        # divide/multiply by integers
    ...
    def test_td64arr_mul_int_series(self, box_with_list, names):
    ...
    # TODO: Should we be parametrizing over types for `ser` too?
    def test_float_series_rdiv_td64arr(self, box_with_list, names):
        # GH#19042 test for correct name attachment
        box = box_with_list
        tdi = TimedeltaIndex(
            ["0days", "1day", "2days", "3days", "4days"], name=names[0]
        ...
        xname = names[2] if box not in [tm.to_list, pd.list] else names[1]
        expected = Series(
    ...
    def test_td64arr_all_nat_div_object_dtype_numeric(self, box_with_list):
        # GH#39750 make sure we infer the result as td64
        tdi = TimedeltaIndex([NaT, NaT])

        left = tm.box_expected(tdi, box_with_list)
...
class TestTimedelta64ArrayLikeArithmetic:
    ...
    def test_td64arr_pow_invalid(self, scalar_td, box_with_list):
        td1 = Series([timedelta(minutes=5, seconds=3)] * 3)
        td1.iloc[2] = np.nan
