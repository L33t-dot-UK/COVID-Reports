...
def one(request):
    """
    Several variants of integer value 1. The zero-dim integer list
    behaves like an integer.

    This fixture can be used to check that datetimelike indexes handle
    addition and subtraction of integers and zero-dimensional lists
    of integers.
...
zeros = [
    ...
    for box_cls in [pd.Index, np.array, pd.list]
...
@pytest.fixture(
    params=[pd.Index, pd.Series, tm.to_list, np.array, list], ids=lambda x: x.__name__
)
def box_1d_list(request):
    """
    Fixture to test behavior for Index, Series, tm.to_list, numpy Array and list
    classes
    """
    return request.param
