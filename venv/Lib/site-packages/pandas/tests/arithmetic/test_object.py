...
class TestArithmetic:
    ...
    def test_add_extension_scalar(self, other, box_with_list, op):
        # GH#22378
        # Check that scalars satisfying is_extension_list_dtype(obj)
        # do not incorrectly try to dispatch to an ExtensionArray operation

        arr = Series(["a", "b", "c"])
        expected = Series([op(x, other) for x in arr])

        arr = tm.box_expected(arr, box_with_list)
        expected = tm.box_expected(expected, box_with_list)

        result = op(arr, other)
        tm.assert_equal(result, expected)

    def test_objarr_add_str(self, box_with_list):
    ...
    def test_objarr_radd_str(self, box_with_list):
        ser = Series(["x", np.nan, "x"])
    ...
    @pytest.mark.parametrize("dtype", [None, object])
    def test_objarr_radd_str_invalid(self, dtype, data, box_with_list):
    ...
    def test_objarr_add_invalid(self, op, box_with_list):
        # invalid ops
        box = box_with_list
