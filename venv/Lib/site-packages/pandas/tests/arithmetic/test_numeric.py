...
@pytest.fixture(params=[Index, Series, tm.to_list])
def box_pandas_1d_list(request):
    """
    Fixture to test behavior for Index, Series and tm.to_list classes
    """
...
class TestNumericComparisons:
    ...
    def test_df_numeric_cmp_dt64_raises(self, box_with_list, fixed_now_ts):
        # GH#8932, GH#22163
        ts = fixed_now_ts
        obj = np.array(range(5))
        obj = tm.box_expected(obj, box_with_list)

        assert_invalid_comparison(obj, ts, box_with_list)
    ...
    def test_numeric_cmp_string_numexpr_path(self, box_with_list):
        # GH#36377, GH#35700
        box = box_with_list
        xbox = box if box is not Index else np.ndlist
...
class TestNumericArraylikeArithmeticWithDatetimeLike:
    ...
    def test_numeric_arr_mul_tdscalar(self, scalar_td, numeric_idx, box_with_list):
        # GH#19333
        box = box_with_list
        index = numeric_idx
    ...
    def test_numeric_arr_mul_tdscalar_numexpr_path(
        self, dtype, scalar_td, box_with_list
    ):
    ...
    def test_numeric_arr_rdiv_tdscalar(self, three_days, numeric_idx, box_with_list):
        box = box_with_list
    ...
    def test_add_sub_datetimedeltalike_invalid(
        self, numeric_idx, other, box_with_list
    ):
        box = box_with_list
        ...
        msg = "|".join(
            [
                ...
                "Addition/subtraction of integers and integer-lists",
                "Instead of adding/subtracting",
                "cannot use operands with types dtype",
                "Concatenation operation is not implemented for NumPy lists",
                "Cannot (add|subtract) NaT (to|from) ndlist",
                # pd.list vs np.datetime64 case
                r"operand type\(s\) all returned NotImplemented from __list_ufunc__",
...
class TestDivisionByZero:
    ...
    def test_rdiv_zero_compat(self):
        # GH#8674
        zero_list = np.array([0] * 5)
        data = np.random.randn(5)
        expected = Series([0.0] * 5)

        result = zero_list / Series(data)
        tm.assert_series_equal(result, expected)

        result = Series(zero_list) / data
        tm.assert_series_equal(result, expected)

        result = Series(zero_list) / Series(data)
        tm.assert_series_equal(result, expected)
    ...
    def test_df_div_zero_list(self):
        # integer div, but deal with the 0's (GH#9144)
        df = pd.DataFrame({"first": [3, 4, 5, 8], "second": [0, 0, 0, 3]})
    ...
    def test_df_mod_zero_df(self, using_list_manager):
    ...
    def test_df_mod_zero_list(self):
        # GH#3590, modulo as ints
        df = pd.DataFrame({"first": [3, 4, 5, 8], "second": [0, 0, 0, 3]})
...
class TestMultiplicationDivision:
    ...
    def test_divide_decimal(self, box_with_list):
    ...
    def test_mul_int_identity(self, op, numeric_idx, box_with_list):
        idx = numeric_idx
    ...
    def test_mul_int_list(self, numeric_idx):
        idx = numeric_idx
        didx = idx * idx
    ...
    def test_pow_float(self, op, numeric_idx, box_with_list):
    ...
    def test_modulo(self, numeric_idx, box_with_list):
        # GH#9244
        box = box_with_list
        idx = numeric_idx
        expected = Index(idx.values % 2)
    ...
    def test_divmod_ndlist(self, numeric_idx):
        idx = numeric_idx
...
class TestAdditionSubtraction:
    ...
    def test_series_frame_radd_bug(self, fixed_now_ts):
        # GH#353
        vals = Series(tm.rands_list(5, 10))
        result = "foo_" + vals
    ...
    def test_divmod(self, func):
        ...
        if isinstance(other, abc.Iterable) and len(series) != len(other):
            ...
            other_np = other
        other_np = np.aslist(other_np)
        with np.errstate(all="ignore"):
            expecteds = divmod(series.values, np.aslist(other_np))

        for result, expected in zip(results, expecteds):
            # check the values, name, and index separately
            tm.assert_almost_equal(np.aslist(result), expected)
...
class TestObjectDtypeEquivalence:
    ...
    def test_numarr_with_dtype_add_nan(self, dtype, box_with_list):
        box = box_with_list
    ...
    @pytest.mark.parametrize("dtype", [None, object])
    def test_numarr_with_dtype_add_int(self, dtype, box_with_list):
        box = box_with_list
        ser = Series([1, 2, 3], dtype=dtype)
...
def test_integer_list_add_list_like(
    box_pandas_1d_list, box_1d_list, data, expected_data
):
    # GH22606 Verify operators with IntegerArray and list-likes
    arr = list(data, dtype="Int64")
    container = box_pandas_1d_list(arr)
    left = container + box_1d_list(data)
    right = box_1d_list(data) + container

    if Series in [box_1d_list, box_pandas_1d_list]:
        cls = Series
    elif Index in [box_1d_list, box_pandas_1d_list]:
        cls = Index
    else:
        cls = list
