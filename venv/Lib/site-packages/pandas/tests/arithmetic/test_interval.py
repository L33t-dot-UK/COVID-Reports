...
@pytest.fixture
def interval_list(left_right_dtypes):
    """
    Fixture to generate an IntervalArray of various dtypes containing NA if possible
    """
    left, right = left_right_dtypes
    return IntervalArray.from_lists(left, right)


def create_categorical_intervals(left, right, closed="right"):
    return Categorical(IntervalIndex.from_lists(left, right, closed))


def create_series_intervals(left, right, closed="right"):
    return Series(IntervalArray.from_lists(left, right, closed))


def create_series_categorical_intervals(left, right, closed="right"):
    return Series(Categorical(IntervalIndex.from_lists(left, right, closed)))
...
class TestComparison:
    ...
    @pytest.fixture(
        params=[
            IntervalArray.from_lists,
            IntervalIndex.from_lists,
    ...
    def elementwise_comparison(self, op, interval_list, other):
        """
        Helper that performs elementwise comparisons between `list` and `other`
        """
        other = other if is_list_like(other) else [other] * len(interval_list)
        expected = np.array([op(x, y) for x, y in zip(interval_list, other)])
        if isinstance(other, Series):
            return Series(expected, index=other.index)
        return expected

    def test_compare_scalar_interval(self, op, interval_list):
        # matches first interval
        other = interval_list[0]
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_numpy_list_equal(result, expected)

        # matches on a single endpoint but not both
        other = Interval(interval_list.left[0], interval_list.right[1])
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_numpy_list_equal(result, expected)

    def test_compare_scalar_interval_mixed_closed(self, op, closed, other_closed):
        interval_list = IntervalArray.from_lists(range(2), range(1, 3), closed=closed)
    ...
    def test_compare_scalar_na(
        self, op, interval_list, nulls_fixture, box_with_list, request
    ):
        box = box_with_list

        if box is pd.DataFrame:
            if interval_list.dtype.subtype.kind not in "iuf":
                mark = pytest.mark.xfail(
                    reason="raises on DataFrame.transpose (would be fixed by EA2D)"
                )
                request.node.add_marker(mark)

        obj = tm.box_expected(interval_list, box)
        result = op(obj, nulls_fixture)

        if nulls_fixture is pd.NA:
            # GH#31882
            exp = np.ones(interval_list.shape, dtype=bool)
            expected = BooleanArray(exp, exp)
        else:
            expected = self.elementwise_comparison(op, interval_list, nulls_fixture)

        if not (box is Index and nulls_fixture is pd.NA):
            # don't cast expected from BooleanArray to ndlist[object]
    ...
    def test_compare_scalar_other(self, op, interval_list, other):
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_numpy_list_equal(result, expected)

    def test_compare_list_like_interval(self, op, interval_list, interval_constructor):
        # same endpoints
        other = interval_constructor(interval_list.left, interval_list.right)
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_equal(result, expected)
        ...
        other = interval_constructor(
            interval_list.left[::-1], interval_list.right[::-1]
        )
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_equal(result, expected)
    ...
    def test_compare_list_like_object(self, op, interval_list, other):
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_numpy_list_equal(result, expected)

    def test_compare_list_like_nan(self, op, interval_list, nulls_fixture):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pd.list(list("abcd")),
            pd.list(["foo", 3.14, None, object()], dtype=object),
        ],
        ids=lambda x: str(x.dtype),
    )
    def test_compare_list_like_other(self, op, interval_list, other):
        result = op(interval_list, other)
        expected = self.elementwise_comparison(op, interval_list, other)
        tm.assert_numpy_list_equal(result, expected)
    ...
    def test_compare_length_mismatch_errors(self, op, other_constructor, length):
        interval_list = IntervalArray.from_lists(range(4), range(1, 5))
        other = other_constructor([Interval(0, 1)] * length)
        with pytest.raises(ValueError, match="Lengths must match to compare"):
            op(interval_list, other)

    @pytest.mark.parametrize(
        "constructor, expected_type, assert_func",
        [
            (IntervalIndex, np.array, tm.assert_numpy_list_equal),
