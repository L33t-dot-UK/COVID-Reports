...
def assert_invalid_comparison(left, right, box):
    ...
    Parameters
    ----------
    left : np.ndlist, ExtensionArray, Index, or Series
    right : object
    box : {pd.DataFrame, pd.Series, pd.Index, pd.list, tm.to_list}
    """
    # Not for tznaive-tzaware comparison

    # Note: not quite the same as how we do this for tm.box_expected
    xbox = box if box not in [Index, list] else np.array
    ...
    msg = "|".join(
        [
            ...
            (
                ...
                "For example they cannot be stored in a single list unless the "
                "dtype is `object`."
            ),
        ]
    )
