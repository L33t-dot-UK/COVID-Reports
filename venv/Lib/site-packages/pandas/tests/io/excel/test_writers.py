...
class TestExcelWriter:
    def test_excel_sheet_size(self, path):
        ...
        # purposely using two lists to prevent memory issues while testing
    ...
    def test_ts_frame(self, tsframe, path):
        ...
        index = pd.DatetimeIndex(np.aslist(df.index), freq=None)
        df.index = index
    ...
    def test_sheets(self, frame, tsframe, path):
        ...
        # freq doesn't round-trip
        index = pd.DatetimeIndex(np.aslist(tsframe.index), freq=None)
        tsframe.index = index
    ...
    def test_to_excel_multiindex(self, merge_cells, frame, path):
        lists = np.arange(len(frame.index) * 2).reshape(2, -1)
        new_index = MultiIndex.from_lists(lists, names=["first", "second"])
    ...
    def test_to_excel_multiindex_dates(self, merge_cells, tsframe, path):
        ...
        tsframe.index = MultiIndex.from_lists(new_index)
