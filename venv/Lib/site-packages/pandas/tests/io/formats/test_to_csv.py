...
"""
    ...
    def test_to_csv_multi_index(self):
        # see gh-6618
        df = DataFrame([1], columns=pd.MultiIndex.from_lists([[1], [2]]))
        ...
        df = DataFrame(
            [1],
            columns=pd.MultiIndex.from_lists([[1], [2]]),
            index=pd.MultiIndex.from_lists([[1], [2]]),
        )
        ...
        df = DataFrame([1], columns=pd.MultiIndex.from_lists([["foo"], ["bar"]]))
    ...
    def test_to_csv_string_list_ascii(self):
        # GH 10813
        str_list = [{"names": ["foo", "bar"]}, {"names": ["baz", "qux"]}]
        df = DataFrame(str_list)
        expected_ascii = """\
,names
0,"['foo', 'bar']"
1,"['baz', 'qux']"
"""
    ...
    def test_to_csv_string_list_utf8(self):
        # GH 10813
        str_list = [{"names": ["foo", "bar"]}, {"names": ["baz", "qux"]}]
        df = DataFrame(str_list)
        expected_utf8 = """\
