...
class TestStyler:
    ...
    def test_bad_apply_shape(self):
        ...
        msg = "returned ndlist with wrong shape"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_apply_bad_return(self):
        ...
        msg = (
            "must return a DataFrame or ndlist when passed to `Styler.apply` "
            "with axis=None"
        )
        with pytest.raises(TypeError, match=msg):
    ...
    def test_get_level_lengths_un_sorted(self):
        index = MultiIndex.from_lists([[1, 1, 2, 1], ["a", "b", "b", "d"]])
        expected = {
            (0, 0): 2,
            (0, 2): 1,
            (0, 3): 1,
    ...
    def test_hide_columns_index_mult_levels(self):
        ...
        i1 = MultiIndex.from_lists(
            [["a", "a"], [0, 1]], names=["idx_level_0", "idx_level_1"]
        )
        i2 = MultiIndex.from_lists(
...
def test_get_level_lengths_mi_hidden():
    # GH 43464
    index = MultiIndex.from_lists([[1, 1, 1, 2, 2, 2], ["a", "a", "b", "a", "a", "b"]])
    expected = {
        (0, 2): 1,
        (0, 3): 1,
        (0, 4): 1,
