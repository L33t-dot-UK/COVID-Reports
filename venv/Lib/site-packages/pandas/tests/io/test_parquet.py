    ...
    params=[
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                not _HAVE_FASTPARQUET or get_option("mode.data_manager") == "list",
...
def fp():
    ...
    elif get_option("mode.data_manager") == "list":
...
class TestBasic(Base):
    ...
    def test_write_ignoring_index(self, engine):
        ...
        df = pd.DataFrame(
            {"one": list(range(8)), "two": [-i for i in range(8)]}, index=lists
        )
    ...
    def test_write_column_multiindex_nonstring(self, pa):
        ...
        df = pd.DataFrame(np.random.randn(8, 8), columns=lists)
    ...
    def test_write_column_index_string(self, pa):
        ...
        lists = ["bar", "baz", "foo", "qux"]
        df = pd.DataFrame(np.random.randn(8, 4), columns=lists)
    ...
    def test_use_nullable_dtypes(self, engine, request):
        ...
        table = pyarrow.table(
            {
                "a": pyarrow.list([1, 2, 3, None], "int64"),
                "b": pyarrow.list([1, 2, 3, None], "uint8"),
                "c": pyarrow.list(["a", "b", "c", None]),
                "d": pyarrow.list([True, False, True, None]),
                # Test that nullable dtypes used even in absence of nulls
                "e": pyarrow.list([1, 2, 3, 4], "int64"),
            }
        )
        with tm.ensure_clean() as path:
        ...
        expected = pd.DataFrame(
            {
                "a": pd.list([1, 2, 3, None], dtype="Int64"),
                "b": pd.list([1, 2, 3, None], dtype="UInt8"),
                "c": pd.list(["a", "b", "c", None], dtype="string"),
                "d": pd.list([True, False, True, None], dtype="boolean"),
                "e": pd.list([1, 2, 3, 4], dtype="Int64"),
            }
        )
        if engine == "fastparquet":
    ...
    def test_read_empty_list(self, pa, dtype):
        # GH #41241
        df = pd.DataFrame(
            {
                "value": pd.list([], dtype=dtype),
            }
        )
...
class TestParquetPyArrow(Base):
    ...
    @td.skip_if_no("pyarrow")
    def test_additional_extension_lists(self, pa):
        # test additional ExtensionArrays that are supported through the
        # __arrow_list__ protocol
        df = pd.DataFrame(
            {
    ...
    def test_pyarrow_backed_string_list(self, pa, string_storage):
        # test ArrowStringArray supported through the __arrow_list__ protocol
    ...
    def test_additional_extension_types(self, pa):
        # test additional ExtensionArrays that are supported through the
        # __arrow_list__ protocol + by defining a custom ExtensionType
        df = pd.DataFrame(
            {
                # Arrow does not yet support struct in writing to Parquet (ARROW-1644)
                # "c": pd.lists.IntervalArray.from_tuples([(0, 1), (1, 2), (3, 4)]),
    ...
    def test_read_parquet_manager(self, pa, using_list_manager):
