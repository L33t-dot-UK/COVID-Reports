...
class TestUltraJSONTests:
    ...
    def test_encode_list_of_nested_lists(self):
        nested_input = [[[[]]]] * 20
    ...
    def test_encode_list_of_doubles(self):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("[[[true", "Unexpected character found when decoding list value (2)"),
        ],
    )
...
class TestNumpyJSONTests:
    ...
    def test_bool_list(self):
        ...
        output = np.array(ujson.decode(ujson.encode(bool_list)), dtype=bool)
        tm.assert_numpy_list_equal(bool_list, output)
    ...
    def test_int_list(self, any_int_numpy_dtype):
        ...
        tm.assert_numpy_list_equal(arr_input, arr_output)
    ...
    def test_float_list(self, float_numpy_dtype):
    ...
    def test_list_basic(self):
        ...
        tm.assert_numpy_list_equal(np.array(ujson.decode(ujson.encode(arr))), arr)
        tm.assert_numpy_list_equal(ujson.decode(ujson.encode(arr), numpy=True), arr)

    @pytest.mark.parametrize("shape", [(10, 10), (5, 5, 4), (100, 1)])
    def test_list_reshaped(self, shape):
        arr = np.arange(100)
    ...
    def test_0d_list(self):
        # gh-18878
        msg = re.escape("list(1) (0d list) is not JSON serializable at the moment")
        with pytest.raises(TypeError, match=msg):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [["a"], 42],
                ValueError,
                r"Cannot decode multidimensional lists with variable length elements "
                r"to numpy",
                {},
            ),
            (
            ...
            (
                [{"a": 42, "b": 23}, {"c": 17}],
                ValueError,
                r"cannot reshape list of size 3 into shape \(2,1\)",
                {"labelled": True},
            ),
        ],
    )
    def test_list_numpy_except(self, bad_input, exc_type, err_msg, kwargs):
        with pytest.raises(exc_type, match=err_msg):
            ujson.decode(ujson.dumps(bad_input), numpy=True, **kwargs)

    def test_list_numpy_labelled(self):
        labelled_input = {"a": []}
...
class TestPandasJSONTests:
    ...
    def test_decode_invalid_list(self, invalid_arr):
        msg = (
            "Expected object or value|Trailing data|"
            "Unexpected character found when decoding list value"
        )
        with pytest.raises(ValueError, match=msg):
            ujson.decode(invalid_arr)

    @pytest.mark.parametrize("arr", [[], [31337]])
    def test_decode_list(self, arr):
    ...
    def test_decode_list_with_big_int(self, value):
        with pytest.raises(
            ValueError,
            match="Value is too big|Value is too small",
        ):
