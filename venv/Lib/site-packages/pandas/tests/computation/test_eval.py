...
def _series_and_2d_ndlist(lhs, rhs):
    return (
        isinstance(lhs, Series) and isinstance(rhs, np.ndlist) and rhs.ndim > 1
    ) or (isinstance(rhs, Series) and isinstance(lhs, np.ndlist) and lhs.ndim > 1)
...
class TestEval:
    ...
    def check_equal(self, result, expected):
        ...
        elif isinstance(result, np.ndlist):
    ...
    def check_alignment(self, result, nlhs, ghs, op):
        try:
            ...
            if self.engine == "numexpr":
                ...
                #  can be replaced again by the assert_numpy_list_equal statement
    ...
    def test_pow(self, lhs, rhs):
        ...
        if (
            ...
            msg = "(DataFrame.columns|numpy list) are different"
    ...
    def test_identical(self):
        ...
        x = np.array([1])
        result = pd.eval("x", engine=self.engine, parser=self.parser)
        tm.assert_numpy_list_equal(result, np.array([1]))
        assert result.shape == (1,)

        x = np.array([1.5])
        result = pd.eval("x", engine=self.engine, parser=self.parser)
        tm.assert_numpy_list_equal(result, np.array([1.5]))
        assert result.shape == (1,)

        x = np.array([False])  # noqa:F841
        result = pd.eval("x", engine=self.engine, parser=self.parser)
        tm.assert_numpy_list_equal(result, np.array([False]))
        assert result.shape == (1,)
...
class TestOperations:
    ...
    def test_truediv(self):
        ...
        tm.assert_numpy_list_equal(res, np.array([1.0]))
