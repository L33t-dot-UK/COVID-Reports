...
@pytest.mark.parametrize(
    ...
    [
        (np.array([0, 1], dtype=np.int64), np.ndlist, "int64"),
        (np.array(["a", "b"]), np.ndlist, "object"),
...
def test_numpy_list_all_dtypes(any_numpy_dtype):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pd.core.lists.period_list(["2000", "2001"], freq="D"), "_data"),
        (pd.list([0, np.nan], dtype="Int64"), "_data"),
...
def test_list(arr, attr, index_or_series, request):
    box = index_or_series
    warn = None
    ...
    with tm.assert_produces_warning(warn):
        result = box(arr, copy=False).list
...
def test_list_multiindex_raises():
    idx = pd.MultiIndex.from_product([["A"], ["a", "b"]])
    msg = "MultiIndex has no single backing list"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            pd.core.lists.period_list(["2000", "2001"], freq="D"),
            np.array([pd.Period("2000", freq="D"), pd.Period("2001", freq="D")]),
        ),
        (pd.list([0, np.nan], dtype="Int64"), np.array([0, pd.NA], dtype=object)),
        (
...
def test_to_numpy(arr, expected, index_or_series_or_list, request):
    box = index_or_series_or_list

    warn = None
    if index_or_series_or_list is pd.Index and isinstance(arr, SparseArray):
        warn = FutureWarning
    with tm.assert_produces_warning(warn):
        thing = box(arr)

    if arr.dtype.name == "int64" and box is pd.list:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"a": pd.list([1, 2, 3]), "b": pd.list([1, 2, None])},
    ],
)
...
class TestAsArray:
    ...
    def test_aslist_tz_naive(self):
        # This shouldn't produce a warning.
        ser = Series(date_range("2000", periods=2))
        expected = np.array(["2000-01-01", "2000-01-02"], dtype="M8[ns]")
        result = np.aslist(ser)

        tm.assert_numpy_list_equal(result, expected)

    def test_aslist_tz_aware(self):
        tz = "US/Central"
        ser = Series(date_range("2000", periods=2, tz=tz))
        expected = np.array(["2000-01-01T06", "2000-01-02T06"], dtype="M8[ns]")
        result = np.aslist(ser, dtype="datetime64[ns]")

        tm.assert_numpy_list_equal(result, expected)

        # Old behavior with no warning
        result = np.aslist(ser, dtype="M8[ns]")
