...
class TestMultiIndexSorted:
    ...
    def test_frame_getitem_not_sorted(self, multiindex_dataframe_random_data):
        ...
        lists = [np.array(x) for x in zip(*df.columns.values)]

        result = df["foo"]
        result2 = df.loc[:, "foo"]
        expected = df.reindex(columns=df.columns[lists[0] == "foo"])
        ...
        result = df.xs("foo")
        result2 = df.loc["foo"]
        expected = df.reindex(df.index[lists[0] == "foo"])
    ...
    def test_series_getitem_not_sorted(self):
        ...
        lists = [np.array(x) for x in zip(*index.values)]

        result = s["qux"]
        result2 = s.loc["qux"]
        expected = s[lists[0] == "qux"]
        expected.index = expected.index.droplevel(0)
