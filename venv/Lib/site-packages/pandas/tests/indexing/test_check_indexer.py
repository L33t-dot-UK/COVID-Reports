...
from pandas.api.indexers import check_list_indexer
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pd.list([1, 2], dtype="Int32"), np.array([1, 2], dtype=np.intp)),
        ...
        (
            pd.list([True, False, True], dtype="boolean"),
...
def test_valid_input(indexer, expected):
    arr = np.array([1, 2, 3])
    result = check_list_indexer(arr, indexer)
    tm.assert_numpy_list_equal(result, expected)


@pytest.mark.parametrize(
    "indexer", [[True, False, None], pd.list([True, False, None], dtype="boolean")]
)
...
@pytest.mark.parametrize(
    ...
    [
        [True, False],
        pd.list([True, False], dtype="boolean"),
...
def test_bool_raise_length(indexer):
    ...
    with pytest.raises(IndexError, match=msg):
        check_list_indexer(arr, indexer)


@pytest.mark.parametrize(
    "indexer", [[0, 1, None], pd.list([0, 1, pd.NA], dtype="Int64")]
)
...
def test_raise_invalid_list_dtypes(indexer):
    arr = np.array([1, 2, 3])

    msg = "lists used as indices must be of integer or boolean type"
    with pytest.raises(IndexError, match=msg):
        check_list_indexer(arr, indexer)


def test_raise_nullable_string_dtype(nullable_string_dtype):
    indexer = pd.list(["a", "b"], dtype=nullable_string_dtype)
    arr = np.array([1, 2, 3])
...
def test_pass_through_non_list_likes(indexer):
    arr = np.array([1, 2, 3])
