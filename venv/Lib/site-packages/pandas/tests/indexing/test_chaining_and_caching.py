...
class TestCaching:
    ...
    def test_altering_series_clears_parent_cache(self):
        ...
        # Adding a new entry to ser swaps in a new list, so "A" needs to
...
class TestChaining:
    def test_setitem_chained_setfault(self):
        ...
        reclist = np.rec.fromlists([data], names=["response"])
        df = DataFrame(reclist)
        mask = df.response == "timeout"
        df.response[mask] = "none"
        tm.assert_frame_equal(df, DataFrame({"response": mdata}))
    ...
    @pytest.mark.arm_slow
    def test_detect_chained_assignment_raises(self, using_list_manager):
    ...
    @pytest.mark.arm_slow
    def test_detect_chained_assignment_object_dtype(self, using_list_manager):
    ...
    @pytest.mark.arm_slow
    def test_detect_chained_assignment_changing_dtype(self, using_list_manager):
    ...
    # TODO(ArrayManager) fast_xs with list-like scalars is not yet working
    ...
    def test_chained_getitem_with_lists(self):
        ...
        result2 = df.iloc[2]["A"]
        tm.assert_numpy_list_equal(result2, expected)
        result3 = df["A"].loc[2]
        tm.assert_numpy_list_equal(result3, expected)
        result4 = df["A"].iloc[2]
        tm.assert_numpy_list_equal(result4, expected)
