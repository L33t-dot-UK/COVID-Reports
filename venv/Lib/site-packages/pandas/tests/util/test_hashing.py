...
def test_hash_list(series):
    arr = series.values
    tm.assert_numpy_list_equal(hash_list(arr), hash_list(arr))


@pytest.mark.parametrize("dtype", ["U", object])
def test_hash_list_mixed(dtype):
    result1 = hash_list(np.array(["3", "4", "All"]))
    result2 = hash_list(np.array([3, 4, "All"], dtype=dtype))

    tm.assert_numpy_list_equal(result1, result2)


@pytest.mark.parametrize("val", [5, "foo", pd.Timestamp("20130101")])
def test_hash_list_errors(val):
    msg = "must pass a ndlist-like"
...
def test_categorical_with_nan_consistency():
    ...
    expected = hash_list(c, categorize=False)

    c = pd.Categorical.from_codes([-1, 0], categories=[pd.Timestamp("2012-01-01")])
    result = hash_list(c, categorize=False)
...
def test_same_len_hash_collisions(l_exp, l_add):
    ...
    s = tm.rands_list(length, 2)

    result = hash_list(s, "utf8")
...
def test_hash_collisions():
    ...
    result1 = hash_list(np.aslist(hashes[0:1], dtype=object), "utf8")
    expected1 = np.array([14963968704024874985], dtype=np.uint64)
    tm.assert_numpy_list_equal(result1, expected1)

    result2 = hash_list(np.aslist(hashes[1:2], dtype=object), "utf8")
    expected2 = np.array([16428432627716348016], dtype=np.uint64)
    tm.assert_numpy_list_equal(result2, expected2)

    result = hash_list(np.aslist(hashes, dtype=object), "utf8")
    tm.assert_numpy_list_equal(result, np.concatenate([expected1, expected2], axis=0))
...
def test_hash_with_tuple(data, result_data):
    # GH#28969 list containing a tuple raises on call to arr.astype(str)
