...
class TestDataFrameToXArray:
    ...
    def test_to_xlist_index_types(self, index_flat, df):
        index = index_flat
        # MultiIndex is tested in test_to_xlist_with_multiindex
        if len(index) == 0:
            pytest.skip("Test doesn't make sense for empty index")

        from xlist import Dataset
    ...
    def test_to_xlist_empty(self, df):
        from xlist import Dataset

        df.index.name = "foo"
        result = df[0:0].to_xlist()
        assert result.dims["foo"] == 0
        assert isinstance(result, Dataset)

    def test_to_xlist_with_multiindex(self, df):
        from xlist import Dataset
...
class TestSeriesToXArray:
    def test_to_xlist_index_types(self, index_flat):
        index = index_flat
        # MultiIndex is tested in test_to_xlist_with_multiindex

        from xlist import DataArray
    ...
    def test_to_xlist_with_multiindex(self):
        from xlist import DataArray
