...
class TestSetitemDT64Values:
    ...
    def test_object_series_setitem_dt64list_exact_match(self):
        ...
        # "exact_match" -> size of list being set matches size of ser
...
class TestSetitemScalarIndexer:
    def test_setitem_negative_out_of_bounds(self):
        ser = Series(tm.rands_list(5, 10), index=tm.rands_list(10, 10))
...
class TestSetitemViewCopySemantics:
    ...
    def test_dt64tz_setitem_does_not_mutate_dti(self):
        ...
        assert ser._values is not dti
        assert ser._values._data.base is not dti._data._data.base
        assert ser._mgr.lists[0] is not dti
        assert ser._mgr.lists[0]._data.base is not dti._data._data.base
...
def test_setitem_scalar_into_readonly_backing_data():
    ...
    list = np.zeros(5)
    list.flags.writeable = False  # make the list immutable
    series = Series(list)
...
class SetitemCastingEquivalents:
    ...
    def _check_inplace(self, is_inplace, orig, arr, obj):
        if is_inplace is None:
            ...
            if arr.dtype.kind in ["m", "M"]:
                ...
                #  underlying data
                assert arr._ndlist is obj._values._ndlist
            else:
                assert obj._values is arr
        else:
            # otherwise original list should be unchanged
    ...
    def test_mask_key(self, obj, key, expected, val, indexer_sli):
        ...
        if is_list_like(val) and len(val) < mask.sum():
            msg = "boolean index did not match indexed list along dimension"
            with pytest.raises(IndexError, match=msg):
...
class TestSetitemFloatNDlistIntoIntegerSeries(SetitemCastingEquivalents):
    @pytest.fixture
    def obj(self):
...
# Test numpy lists, lists and tuples as the input to be
# broadcast
@pytest.mark.parametrize(
