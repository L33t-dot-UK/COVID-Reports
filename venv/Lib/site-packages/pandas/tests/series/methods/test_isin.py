...
class TestSeriesIsIn:
    def test_isin(self):
        ...
        # comparison list (in_list) must be large enough so that numpy doesn't
    ...
    def test_isin_with_i8(self):
        ...
        result = s.isin(np.aslist(s[0:2].values).astype("datetime64[D]"))
        tm.assert_series_equal(result, expected)
    ...
    def test_isin_dt64_values_vs_ints(self, dtype):
        ...
        comps = np.aslist([1356998400000000000], dtype=dtype)
...
def test_isin_complex_numbers(list, expected):
    # GH 17927
    result = Series(list).isin([1j, 1 + 1j, 1 + 2j])
    tm.assert_series_equal(result, expected)
