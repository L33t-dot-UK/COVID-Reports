...
def test_reindex(datetime_series, string_series):
    ...
    # __list_interface__ is not defined for older numpies
    # and on some pythons
    try:
...
def test_reindex_fill_value_datetimelike_upcast(dtype, fill_value, using_list_manager):
...
def test_reindex_empty_with_level(values):
    # GH41170
    ser = Series(
        range(len(values[0])), index=MultiIndex.from_lists(values), dtype="object"
    )
