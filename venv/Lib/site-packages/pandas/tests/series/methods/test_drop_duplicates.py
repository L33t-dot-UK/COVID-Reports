...
class TestSeriesDropDuplicates:
    ...
    def cat_series_unused_category(self, dtype, ordered):
        # Test case 1
        cat_list = np.array([1, 2, 3, 4, 5], dtype=np.dtype(dtype))

        input1 = np.array([1, 2, 3, 3], dtype=np.dtype(dtype))
        cat = Categorical(input1, categories=cat_list, ordered=ordered)
        tc1 = Series(cat)
        return tc1
    ...
    def cat_series(self, dtype, ordered):
        ...
        cat = Categorical(input2, categories=cat_list, ordered=ordered)
        tc2 = Series(cat)
        return tc2
