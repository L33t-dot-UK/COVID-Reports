...
_list = _frame["A"].values.copy()
_list2 = _frame2["A"].values.copy()

_list_mixed = _mixed["D"].values.copy()
_list_mixed2 = _mixed2["D"].values.copy()
...
class TestExpressions:
    ...
    def test_invalid(self):
        list = np.random.randn(1_000_001)
        list2 = np.random.randn(100)

        # no op
        result = expr._can_use_numexpr(operator.add, None, list, list, "evaluate")
        assert not result

        # min elements
        result = expr._can_use_numexpr(operator.add, "+", list2, list2, "evaluate")
        assert not result

        # ok, we only check on first part of expression
        result = expr._can_use_numexpr(operator.add, "+", list, list2, "evaluate")
        assert result
    ...
    @pytest.mark.parametrize(
        "left,right", [(_list, _list2), (_list_mixed, _list_mixed2)]
    )
