...
class TestDataFramePlots(TestPlotBase):
    ...
    def test_integer_list_plot(self):
        # GH 25587
        arr = pd.list([1, 2, 3, 4], dtype="UInt32")
    ...
    def _compare_stacked_y_cood(self, normal_lines, stacked_lines):
        ...
        for nl, sl in zip(normal_lines, stacked_lines):
            ...
            sy = sl.get_data()[1]
            tm.assert_numpy_list_equal(base, sy)
    ...
    def test_line_area_nan_df(self):
        ...
        for d in [df, tdf]:
            ...
            tm.assert_numpy_list_equal(np.delete(masked1.data, 2), exp)

            exp = np.array([3, 2, 1], dtype=np.float64)
            tm.assert_numpy_list_equal(np.delete(masked2.data, 1), exp)
            ...
            tm.assert_numpy_list_equal(ax.lines[0].get_ydata(), expected1)
            tm.assert_numpy_list_equal(ax.lines[1].get_ydata(), expected1 + expected2)
            ...
            tm.assert_numpy_list_equal(ax.lines[0].get_ydata(), expected1)
            tm.assert_numpy_list_equal(ax.lines[1].get_ydata(), expected2)
    ...
    def test_bar_categorical(self):
        ...
        for df in [df1, df2]:
            ...
            tm.assert_numpy_list_equal(ticks, np.array([0, 1, 2, 3, 4, 5]))
    ...
    def test_scatterplot_color_by_categorical(self, ordered, categories):
        ...
        tm.assert_numpy_list_equal(result_ticks, expected_ticks)

        expected_boundaries = np.array([0.0, 1.0, 2.0, 3.0])
        result_boundaries = colorbar._boundaries
        tm.assert_numpy_list_equal(result_boundaries, expected_boundaries)
    ...
    def test_plot_scatter_with_c(self, request):
        ...
        rgba_list = np.array([red_rgba, green_rgba])
        ax = df.plot.scatter(x="A", y="B", c=rgba_list)
        # expect the face colors of the points in the non-colormap path to be
        # identical to the values we supplied, normally we'd be on shaky ground
        # comparing floats for equality but here we expect them to be
        # identical.
        tm.assert_numpy_list_equal(ax.collections[0].get_facecolor(), rgba_list)
        # we don't test the colors of the faces in this next plot because they
        # are dependent on the spring colormap, which may change its colors
        # later.
        float_list = np.array([0.0, 1.0])
        df.plot.scatter(x="A", y="B", c=float_list, cmap="spring")
    ...
    def test_plot_scatter_with_s(self):
        ...
        tm.assert_numpy_list_equal(df["c"].values, right=ax.collections[0].get_sizes())
    ...
    def test_boxplot(self):
        ...
        labels = [pprint_thing(c) for c in numeric_cols]
        self._check_text_labels(ax.get_xticklabels(), labels)
        tm.assert_numpy_list_equal(ax.xaxis.get_ticklocs(), positions)
        assert len(ax.lines) == self.bp_n_objects * len(numeric_cols)
    ...
    def test_boxplot_vertical(self):
        ...
        self._check_text_labels(ax.get_yticklabels(), labels)
        tm.assert_numpy_list_equal(ax.yaxis.get_ticklocs(), positions)
        assert len(ax.lines) == self.bp_n_objects * len(numeric_cols)
    ...
    def _check_box_coord(
        ...
        if expected_y is not None:
            tm.assert_numpy_list_equal(result_y, expected_y, check_dtype=False)
        if expected_h is not None:
            tm.assert_numpy_list_equal(result_height, expected_h, check_dtype=False)
        if expected_x is not None:
            tm.assert_numpy_list_equal(result_x, expected_x, check_dtype=False)
        if expected_w is not None:
            tm.assert_numpy_list_equal(result_width, expected_w, check_dtype=False)
    ...
    def test_unordered_ts(self):
        ...
        tm.assert_numpy_list_equal(ydata, np.array([1.0, 2.0, 3.0]))
