...
class TestTSPlot(TestPlotBase):
    ...
    def test_line_plot_inferred_freq(self):
        for ser in self.datetime_ser:
            ser = Series(ser.values, Index(np.aslist(ser.index)))
    ...
    def test_mixed_freq_irregular_first(self):
        ...
        tm.assert_numpy_list_equal(x1, s2.index.astype(object).values)
        x2 = lines[1].get_xdata()
        tm.assert_numpy_list_equal(x2, s1.index.astype(object).values)
    ...
    def test_from_weekly_resampling(self):
        ...
        for line in ax.get_lines():
            ...
            if len(xdata) == 12:  # idxl lines
                tm.assert_numpy_list_equal(xdata, expected_l)
            else:
                tm.assert_numpy_list_equal(xdata, expected_h)
        tm.close()
    ...
    def test_from_resampling_area_line_mixed(self):
        ...
        for kind1, kind2 in [("line", "area"), ("area", "line")]:
            ...
            for i in range(3):
                ...
                tm.assert_numpy_list_equal(line.get_xdata(orig=False), expected_x)
                # check stacked values are correct
                expected_y += low[i].values
                tm.assert_numpy_list_equal(line.get_ydata(orig=False), expected_y)
        ...
        for kind1, kind2 in [("line", "area"), ("area", "line")]:
            ...
            for i in range(3):
                ...
                tm.assert_numpy_list_equal(lines.get_xdata(orig=False), expected_x)
                expected_y += low[i].values
                tm.assert_numpy_list_equal(lines.get_ydata(orig=False), expected_y)
    ...
    def test_mpl_nopandas(self):
        ...
        tm.assert_numpy_list_equal(line1.get_xydata()[:, 0], exp)
        exp = np.array([x.toordinal() for x in dates], dtype=np.float64)
        tm.assert_numpy_list_equal(line2.get_xydata()[:, 0], exp)
    ...
    def test_add_matplotlib_datetime64(self):
        ...
        l1, l2 = ax.lines
        tm.assert_numpy_list_equal(l1.get_xydata(), l2.get_xydata())
