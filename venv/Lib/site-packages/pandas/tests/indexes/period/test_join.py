...
class TestJoin:
    def test_join_outer_indexer(self):
        ...
        tm.assert_extension_list_equal(result[0], pi._values)
        tm.assert_numpy_list_equal(result[1], np.arange(len(pi), dtype=np.intp))
        tm.assert_numpy_list_equal(result[2], np.arange(len(pi), dtype=np.intp))
