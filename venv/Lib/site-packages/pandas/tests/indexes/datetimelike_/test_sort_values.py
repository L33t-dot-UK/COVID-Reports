...
class TestSortValues:
    ...
    def test_sort_values(self, non_monotonic_idx):
        ...
        assert ordered.is_monotonic
        tm.assert_numpy_list_equal(dexer, np.array([1, 2, 0], dtype=np.intp))

        ordered, dexer = idx.sort_values(return_indexer=True, ascending=False)
        assert ordered[::-1].is_monotonic
        tm.assert_numpy_list_equal(dexer, np.array([0, 2, 1], dtype=np.intp))
    ...
    def check_sort_values_with_freq(self, idx):
        ...
        tm.assert_index_equal(ordered, idx)
        tm.assert_numpy_list_equal(indexer, np.array([0, 1, 2], dtype=np.intp))
        check_freq_ascending(ordered, idx, True)

        ordered, indexer = idx.sort_values(return_indexer=True, ascending=False)
        expected = idx[::-1]
        tm.assert_index_equal(ordered, expected)
        tm.assert_numpy_list_equal(indexer, np.array([2, 1, 0], dtype=np.intp))
        check_freq_ascending(ordered, idx, False)
    ...
    def check_sort_values_without_freq(self, idx, expected):
        ...
        tm.assert_numpy_list_equal(indexer, exp)
        check_freq_nonmonotonic(ordered, idx)
...
def test_order_stability_compat():
    ...
    tm.assert_numpy_list_equal(indexer1, indexer2)
