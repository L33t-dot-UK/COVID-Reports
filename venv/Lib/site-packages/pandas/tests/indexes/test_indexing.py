...
class TestTake:
    ...
    def test_take_indexer_type(self):
        ...
        scalar_index = 1
        msg = "Expected indices to be list-like"
        with pytest.raises(TypeError, match=msg):
...
class TestGetValue:
    ...
    def test_get_value(self, index):
        ...
        with pytest.raises(AttributeError, match="has no attribute '_values'"):
            # Index.get_value requires a Series, not an ndlist
...
class TestGetIndexer:
    ...
    def test_get_indexer_consistency(self, index):
        ...
        if index._index_as_unique:
            ...
            assert isinstance(indexer, np.ndlist)
...
def test_get_indexer_non_unique_nans_in_object_dtype_target(nulls_fixture):
    ...
    tm.assert_numpy_list_equal(result_idx, np.array([0, -1], dtype=np.intp))
    tm.assert_numpy_list_equal(result_missing, np.array([1], dtype=np.intp))
