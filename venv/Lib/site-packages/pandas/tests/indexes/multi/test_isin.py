...
def test_isin_nan():
    idx = MultiIndex.from_lists([["foo", "bar"], [1.0, np.nan]])
    tm.assert_numpy_list_equal(idx.isin([("bar", np.nan)]), np.array([False, True]))
...
def test_isin():
    ...
    idx = MultiIndex.from_lists([["qux", "baz", "foo", "bar"], np.arange(4)])
...
def test_isin_level_kwarg():
    ...
    expected = np.array([False, False, True, True])
    tm.assert_numpy_list_equal(expected, idx.isin(vals_0, level=0))
    tm.assert_numpy_list_equal(expected, idx.isin(vals_0, level=-2))

    tm.assert_numpy_list_equal(expected, idx.isin(vals_1, level=1))
    tm.assert_numpy_list_equal(expected, idx.isin(vals_1, level=-1))
    ...
    idx.names = ["A", "B"]
    tm.assert_numpy_list_equal(expected, idx.isin(vals_0, level="A"))
    tm.assert_numpy_list_equal(expected, idx.isin(vals_1, level="B"))
...
def test_isin_multi_index_with_missing_value(labels, expected, level):
    # GH 19132
    midx = MultiIndex.from_lists([[np.nan, "a", "b"], ["c", "d", np.nan]])
