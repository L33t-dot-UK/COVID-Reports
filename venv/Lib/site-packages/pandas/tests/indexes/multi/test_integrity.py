...
def test_values_boxed():
    ...
    expected = construct_1d_object_list_from_listlike(tuples)
    tm.assert_numpy_list_equal(result.values, expected)
    # Check that code branches for boxed values produce identical results
    tm.assert_numpy_list_equal(result.values[:4], result[:4].values)
...
def test_values_multiindex_datetimeindex():
    ...
    idx = MultiIndex.from_lists([naive, aware])
    result = idx.values
...
def test_values_multiindex_periodindex():
    ...
    idx = MultiIndex.from_lists([ints, pidx])
    result = idx.values
...
def test_hash_collisions():
    ...
    tm.assert_numpy_list_equal(result, np.arange(len(index), dtype="intp"))
...
def test_level_setting_resets_attributes():
    ind = MultiIndex.from_lists([["A", "A", "B", "B", "B"], [1, 2, 1, 2, 3]])
