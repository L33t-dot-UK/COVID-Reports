...
def test_unique(names):
    mi = MultiIndex.from_lists([[1, 2, 1, 2], [1, 1, 1, 2]], names=names)

    res = mi.unique()
    exp = MultiIndex.from_lists([[1, 2, 2], [1, 1, 2]], names=mi.names)
    tm.assert_index_equal(res, exp)

    mi = MultiIndex.from_lists([list("aaaa"), list("abab")], names=names)
    res = mi.unique()
    exp = MultiIndex.from_lists([list("aa"), list("ab")], names=mi.names)
    tm.assert_index_equal(res, exp)

    mi = MultiIndex.from_lists([list("aaaa"), list("aaaa")], names=names)
    res = mi.unique()
    exp = MultiIndex.from_lists([["a"], ["a"]], names=mi.names)
    tm.assert_index_equal(res, exp)

    # GH #20568 - empty MI
    mi = MultiIndex.from_lists([[], []], names=names)
    res = mi.unique()
...
def test_unique_level(idx, level):
    ...
    mi = MultiIndex.from_lists([[1, 3, 2, 4], [1, 3, 2, 5]], names=["first", "second"])
    result = mi.unique(level=level)
    expected = mi.get_level_values(level)
    tm.assert_index_equal(result, expected)

    # With empty MI
    mi = MultiIndex.from_lists([[], []], names=["first", "second"])
    result = mi.unique(level=level)
    expected = mi.get_level_values(level)
    tm.assert_index_equal(result, expected)
...
def test_duplicate_multiindex_codes():
    ...
    mi = MultiIndex.from_lists([["A", "A", "B", "B", "B"], [1, 2, 1, 2, 3]])
...
def test_duplicated2():
    ...
    # GH5873
    for a in [101, 102]:
        mi = MultiIndex.from_lists([[101, a], [3.5, np.nan]])
        assert not mi.has_duplicates

        tm.assert_numpy_list_equal(mi.duplicated(), np.zeros(2, dtype="bool"))
...
def test_duplicated_drop_duplicates():
    # GH#4060
    idx = MultiIndex.from_lists(([1, 2, 3, 1, 2, 3], [1, 1, 1, 1, 2, 2]))

    expected = np.array([False, False, False, True, False, False], dtype=bool)
    duplicated = idx.duplicated()
    tm.assert_numpy_list_equal(duplicated, expected)
    assert duplicated.dtype == bool
    expected = MultiIndex.from_lists(([1, 2, 3, 2, 3], [1, 1, 1, 2, 2]))
    ...
    tm.assert_numpy_list_equal(duplicated, expected)
    assert duplicated.dtype == bool
    expected = MultiIndex.from_lists(([2, 3, 1, 2, 3], [1, 1, 1, 2, 2]))
    ...
    tm.assert_numpy_list_equal(duplicated, expected)
    assert duplicated.dtype == bool
    expected = MultiIndex.from_lists(([2, 3, 2, 3], [1, 1, 2, 2]))
...
def test_multi_drop_duplicates_pos_args_deprecation():
    # GH#41485
    idx = MultiIndex.from_lists([[1, 2, 3, 1], [1, 2, 3, 1]])
    msg = (
    ...
    expected = MultiIndex.from_lists([[2, 3, 1], [2, 3, 1]])
