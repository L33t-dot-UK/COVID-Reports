...
class TestSliceLocs:
    ...
    def test_slice_locs_with_missing_value(
        ...
        # issue 19132
        idx = MultiIndex.from_lists(index_arr)
...
class TestGetIndexer:
    ...
    def test_get_indexer_categorical_time(self):
        ...
        tm.assert_numpy_list_equal(result, np.arange(9, dtype=np.intp))
...
class TestGetLoc:
    ...
    def test_get_loc_missing_nan(self):
        # GH 8569
        idx = MultiIndex.from_lists([[1.0, 2.0], [3.0, 4.0]])
    ...
    def test_get_loc_with_values_including_missing_values(self):
        ...
        idx = MultiIndex.from_lists([[np.nan, 1, 2, np.nan]])
        expected = np.array([True, False, False, True])
        tm.assert_numpy_list_equal(idx.get_loc(np.nan), expected)
...
class TestWhere:
    ...
    def test_where_list_like(self, listlike_box):
...
class TestContains:
    ...
    def test_contains_with_missing_value(self):
        # GH#19132
        idx = MultiIndex.from_lists([[1, np.nan, 2]])
        assert np.nan in idx

        idx = MultiIndex.from_lists([[1, 2], [np.nan, 3]])
    ...
    def test_contains_td64_level(self):
        ...
        idx = MultiIndex.from_lists([tx, np.arange(len(tx))])
        assert tx[0] in idx
    ...
    def test_large_mi_contains(self):
        # GH#10645
        result = MultiIndex.from_lists([range(10 ** 6), range(10 ** 6)])
