...
def test_set_ops_error_cases(idx, case, sort, method):
    # non-iterable input
    msg = "Input must be Index or list-like"
...
def test_union_with_regular_index(idx):
    ...
    msg = "The values in the list are unorderable"
    with tm.assert_produces_warning(RuntimeWarning, match=msg):
...
def test_intersection_non_object(idx, sort):
    ...
    # if we pass a length-0 ndlist (i.e. no name, we retain our idx.name)
    result = idx.intersection(np.aslist(other)[:0], sort=sort)
    ...
    with pytest.raises(TypeError, match=msg):
        ...
        idx.intersection(np.aslist(other), sort=sort)
...
def test_union_empty_self_different_names():
    # GH#38423
    mi = MultiIndex.from_lists([[]])
    mi2 = MultiIndex.from_lists([[1, 2], [3, 4]], names=["a", "b"])
    result = mi.union(mi2)
    expected = MultiIndex.from_lists([[1, 2], [3, 4]])
    tm.assert_index_equal(result, expected)


def test_union_multiindex_empty_rangeindex():
    # GH#41234
    mi = MultiIndex.from_lists([[1, 2], [3, 4]], names=["a", "b"])
    ri = pd.RangeIndex(0)
...
def test_intersection_equal_different_names():
    # GH#30302
    mi1 = MultiIndex.from_lists([[1, 2], [3, 4]], names=["c", "b"])
    mi2 = MultiIndex.from_lists([[1, 2], [3, 4]], names=["a", "b"])

    result = mi1.intersection(mi2)
    expected = MultiIndex.from_lists([[1, 2], [3, 4]], names=[None, "b"])
    tm.assert_index_equal(result, expected)


def test_intersection_different_names():
    # GH#38323
    mi = MultiIndex.from_lists([[1], [3]], names=["c", "b"])
    mi2 = MultiIndex.from_lists([[1], [3]])
...
def test_intersection_with_missing_values_on_both_sides(nulls_fixture):
    # GH#38623
    mi1 = MultiIndex.from_lists([[3, nulls_fixture, 4, nulls_fixture], [1, 2, 4, 2]])
    mi2 = MultiIndex.from_lists([[3, nulls_fixture, 3], [1, 2, 4]])
    result = mi1.intersection(mi2)
    expected = MultiIndex.from_lists([[3.0, nulls_fixture], [1, 2]])
    tm.assert_index_equal(result, expected)


def test_union_nan_got_duplicated():
    # GH#38977
    mi1 = MultiIndex.from_lists([[1.0, np.nan], [2, 3]])
    mi2 = MultiIndex.from_lists([[1.0, np.nan, 3.0], [2, 3, 4]])
...
def test_union_duplicates(index):
    ...
    mi1 = MultiIndex.from_lists([values, [1] * len(values)])
    mi2 = MultiIndex.from_lists([[values[0]] + values, [1] * (len(values) + 1)])
    result = mi1.union(mi2)
