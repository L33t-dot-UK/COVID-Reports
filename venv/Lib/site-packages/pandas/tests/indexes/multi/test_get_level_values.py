...
def test_get_level_values_all_na():
    ...
    lists = [[np.nan, np.nan, np.nan], ["a", np.nan, 1]]
    index = MultiIndex.from_lists(lists)
...
def test_get_level_values_int_with_na():
    # GH#17924
    lists = [["a", "b", "b"], [1, np.nan, 2]]
    index = MultiIndex.from_lists(lists)
    result = index.get_level_values(1)
    expected = Index([1, np.nan, 2])
    tm.assert_index_equal(result, expected)

    lists = [["a", "b", "b"], [np.nan, np.nan, 2]]
    index = MultiIndex.from_lists(lists)
    result = index.get_level_values(1)
...
def test_get_level_values_na():
    ...
    lists = [["a", "b", "b"], pd.DatetimeIndex([0, 1, pd.NaT])]
    index = MultiIndex.from_lists(lists)
    result = index.get_level_values(1)
