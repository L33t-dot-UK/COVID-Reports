...
def test_get_loc_partial_timestamp_multiindex(df):
    ...
    expected2[[6, 9]] = True
    tm.assert_numpy_list_equal(loc2, expected2)

    key3 = ("2016-01", "a")
    loc3 = mi.get_loc(key3)
    expected3 = np.zeros(len(mi), dtype=bool)
    expected3[mi.get_level_values(1).get_loc("a")] = True
    tm.assert_numpy_list_equal(loc3, expected3)

    key4 = ("2016", "a")
    loc4 = mi.get_loc(key4)
    expected4 = expected3
    tm.assert_numpy_list_equal(loc4, expected4)
    ...
    expected5[[3, 14]] = True
    tm.assert_numpy_list_equal(loc5, expected5)
