...
class TestFloatNumericIndex(NumericBase):
    ...
    def test_constructor(self, dtype):
        ...
        tm.assert_numpy_list_equal(index.values, expected)
    ...
    def test_nan_multiple_containment(self):
        ...
        idx = index_cls([1.0, np.nan])
        tm.assert_numpy_list_equal(idx.isin([1.0]), np.array([True, False]))
        tm.assert_numpy_list_equal(idx.isin([2.0, np.pi]), np.array([False, False]))
        tm.assert_numpy_list_equal(idx.isin([np.nan]), np.array([False, True]))
        tm.assert_numpy_list_equal(idx.isin([1.0, np.nan]), np.array([True, True]))
        idx = index_cls([1.0, 2.0])
        tm.assert_numpy_list_equal(idx.isin([np.nan]), np.array([False, False]))
