...
def test_rolling_forward_window(constructor, func, np_func, expected, np_kwargs):
    ...
    # Check that the function output matches the explicitly provided list
...
def test_fixed_forward_indexer_bounds(
    ...
    tm.assert_numpy_list_equal(start, np.array(expected_start), check_dtype=False)
    tm.assert_numpy_list_equal(end, np.array(expected_end), check_dtype=False)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            Series(
                [0, 1.5, 2.0],
                index=MultiIndex.from_lists([[1, 2, 2], range(3)], names=["a", None]),
                name="b",
                dtype=np.float64,
            ),
        ),
