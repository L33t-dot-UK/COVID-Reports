...
class TestExtensionArrayDtype:
    ...
    def test_is_extension_list_dtype(self, values):
        assert is_extension_list_dtype(values)

    @pytest.mark.parametrize("values", [np.array([]), pd.Series(np.array([]))])
    def test_is_not_extension_list_dtype(self, values):
        assert not is_extension_list_dtype(values)
...
def test_is_extension_list_dtype(dtype):
    assert isinstance(dtype, dtypes.ExtensionDtype)
    assert is_extension_list_dtype(dtype)
