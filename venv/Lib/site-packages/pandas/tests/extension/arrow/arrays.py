...
At the moment, just a boolean list / type is implemented.
...
class ArrowBoolDtype(ExtensionDtype):
    ...
    @classmethod
    def construct_list_type(cls) -> type_t[ArrowBoolArray]:
        """
...
class ArrowStringDtype(ExtensionDtype):
    ...
    @classmethod
    def construct_list_type(cls) -> type_t[ArrowStringArray]:
        """
        Return the list type associated with this dtype.
...
class ArrowExtensionArray(OpsMixin, ExtensionArray):
    ...
    def from_scalars(cls, values):
        if isinstance(values, cls):
            # in particular for empty cases the pa.list(np.aslist(...))
            #  does not round-trip
        ...
        arr = pa.chunked_list([pa.list(np.aslist(values))])
        return cls(arr)

    @classmethod
    def from_list(cls, arr):
        assert isinstance(arr, pa.Array)
        return cls(pa.chunked_list([arr]))
    ...
    def _logical_method(self, other, op):
        ...
        return ArrowBoolArray(
            pa.chunked_list([pa.list(result, mask=pd.isna(self._data.to_pandas()))])
        )

    def __eq__(self, other):
        if not isinstance(other, type(self)):
            # TODO: use some pyarrow function here?
            return np.aslist(self).__eq__(other)
