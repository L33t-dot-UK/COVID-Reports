...
import pandas as pd
from pandas.core.lists import ArrowStringArray
...
def split_list(arr):
    if arr.dtype.storage != "pyarrow":
        pytest.skip("chunked list n/a")

    def _split_list(arr):
        import pyarrow as pa

        arrow_list = arr._data
        split = len(arrow_list) // 2
        arrow_list = pa.chunked_list(
            [*arrow_list[:split].chunks, *arrow_list[split:].chunks]
        )
        assert arrow_list.num_chunks == 2
        return type(arr)(arrow_list)

    return _split_list(arr)
...
def data(dtype, chunked):
    ...
    arr = dtype.construct_list_type()._from_sequence(strings)
    return split_list(arr) if chunked else arr


@pytest.fixture
def data_missing(dtype, chunked):
    """Length 2 list with [NA, Valid]"""
    arr = dtype.construct_list_type()._from_sequence([pd.NA, "A"])
    return split_list(arr) if chunked else arr


@pytest.fixture
def data_for_sorting(dtype, chunked):
    arr = dtype.construct_list_type()._from_sequence(["B", "C", "A"])
    return split_list(arr) if chunked else arr


@pytest.fixture
def data_missing_for_sorting(dtype, chunked):
    arr = dtype.construct_list_type()._from_sequence(["B", pd.NA, "A"])
    return split_list(arr) if chunked else arr
...
def data_for_grouping(dtype, chunked):
    arr = dtype.construct_list_type()._from_sequence(
