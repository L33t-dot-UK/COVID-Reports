...
def make_data():
    N = 100
    left_list = np.random.uniform(size=N).cumsum()
    right_list = left_list + np.random.uniform(size=N)
    return [Interval(left, right) for left, right in zip(left_list, right_list)]
