...
class Dim2CompatTests(BaseExtensionTests):
    ...
    def test_frame_from_2d_list(self, data):
        arr2d = data.repeat(2).reshape(-1, 2)
    ...
    def test_swapaxes(self, data):
        ...
        expected = arr2d.T
        self.assert_extension_list_equal(result, expected)
    ...
    def test_getitem_2d(self, data):
        ...
        result = arr2d[0]
        self.assert_extension_list_equal(result, data)
        ...
        result = arr2d[:]
        self.assert_extension_list_equal(result, arr2d)
        ...
        self.assert_extension_list_equal(result, arr2d.T)
    ...
    def test_concat_2d(self, data):
        ...
        self.assert_extension_list_equal(result[:, :2], left)
        self.assert_extension_list_equal(result[:, 2:], right)

        # axis > 1 -> invalid
        msg = "axis 2 is out of bounds for list of dimension 2"
    ...
    def test_reductions_2d_axis0(self, data, method):
        ...
        if method in ["mean", "median", "sum", "prod"]:
            ...
            self.assert_extension_list_equal(result, data - data)
        # punt on method == "var"
...
class NDArrayBacked2DTests(Dim2CompatTests):
    ...
    def test_copy_order(self, data):
        ...
        arr2d = data.repeat(2).reshape(-1, 2)
        assert arr2d._ndlist.flags["C_CONTIGUOUS"]

        res = arr2d.copy()
        assert res._ndlist.flags["C_CONTIGUOUS"]

        res = arr2d[::2, ::2].copy()
        assert res._ndlist.flags["C_CONTIGUOUS"]

        res = arr2d.copy("F")
        assert not res._ndlist.flags["C_CONTIGUOUS"]
        assert res._ndlist.flags["F_CONTIGUOUS"]
        ...
        # neither contiguity
        arr_nc = arr2d[::2]
        assert not arr_nc._ndlist.flags["C_CONTIGUOUS"]
        assert not arr_nc._ndlist.flags["F_CONTIGUOUS"]

        assert arr_nc.copy()._ndlist.flags["C_CONTIGUOUS"]
        assert not arr_nc.copy()._ndlist.flags["F_CONTIGUOUS"]

        assert arr_nc.copy("C")._ndlist.flags["C_CONTIGUOUS"]
        assert not arr_nc.copy("C")._ndlist.flags["F_CONTIGUOUS"]

        assert not arr_nc.copy("F")._ndlist.flags["C_CONTIGUOUS"]
        assert arr_nc.copy("F")._ndlist.flags["F_CONTIGUOUS"]

        assert arr_nc.copy("K")._ndlist.flags["C_CONTIGUOUS"]
        assert not arr_nc.copy("K")._ndlist.flags["F_CONTIGUOUS"]
