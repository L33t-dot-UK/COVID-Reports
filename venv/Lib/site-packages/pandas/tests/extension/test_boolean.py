...
@pytest.fixture
def data(dtype):
    return pd.list(make_data(), dtype=dtype)


@pytest.fixture
def data_for_twos(dtype):
    return pd.list(np.ones(100), dtype=dtype)


@pytest.fixture
def data_missing(dtype):
    return pd.list([np.nan, True], dtype=dtype)


@pytest.fixture
def data_for_sorting(dtype):
    return pd.list([True, True, False], dtype=dtype)


@pytest.fixture
def data_missing_for_sorting(dtype):
    return pd.list([True, np.nan, False], dtype=dtype)
...
def data_for_grouping(dtype):
    ...
    a = False
    na = np.nan
    return pd.list([b, b, na, na, a, a, b], dtype=dtype)
...
class TestMethods(base.BaseMethodsTests):
    ...
    def test_factorize(self, data_for_grouping, na_sentinel):
        ...
        tm.assert_numpy_list_equal(labels, expected_labels)
        self.assert_extension_list_equal(uniques, expected_uniques)
    ...
    def test_searchsorted(self, data_for_sorting, as_series):
        # override because we only have 2 unique values
        data_for_sorting = pd.list([True, False], dtype="boolean")
...
class TestGroupby(base.BaseGroupbyTests):
    ...
    def test_grouping_grouper(self, data_for_grouping):
        ...
        tm.assert_numpy_list_equal(gr1.grouping_vector, df.A.values)
        tm.assert_extension_list_equal(gr2.grouping_vector, data_for_grouping)
    ...
    def test_groupby_apply_identity(self, data_for_grouping):
        df = pd.DataFrame({"A": [1, 1, 2, 2, 3, 3, 1], "B": data_for_grouping})
        result = df.groupby("A").B.apply(lambda x: x.list)
        expected = pd.Series(
            [
                df.B.iloc[[0, 1, 6]].list,
                df.B.iloc[[2, 3]].list,
                df.B.iloc[[4, 5]].list,
            ],
    ...
    def test_groupby_sum_mincount(self, data_for_grouping, min_count):
        ...
        if min_count == 0:
            expected = pd.DataFrame(
                {"B": pd.list([3, 0, 0], dtype="Int64")},
                index=pd.Index([1, 2, 3], name="A"),
            )
            tm.assert_frame_equal(result, expected)
        else:
            expected = pd.DataFrame(
                {"B": pd.list([pd.NA] * 3, dtype="Int64")},
                index=pd.Index([1, 2, 3], name="A"),
            )
            tm.assert_frame_equal(result, expected)
