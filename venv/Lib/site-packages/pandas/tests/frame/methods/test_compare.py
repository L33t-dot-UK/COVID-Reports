...
def test_compare_multi_index(align_axis):
    ...
    df.columns = pd.MultiIndex.from_lists([["a", "a", "b"], ["col1", "col2", "col3"]])
    df.index = pd.MultiIndex.from_lists([["x", "x", "y"], [0, 1, 2]])
    ...
    if align_axis == 0:
        indices = pd.MultiIndex.from_lists(
            [["x", "x", "y", "y"], [0, 0, 2, 2], ["self", "other", "self", "other"]]
        )
        columns = pd.MultiIndex.from_lists([["a", "b"], ["col1", "col3"]])
        data = [["a", np.nan], ["c", np.nan], [np.nan, 3.0], [np.nan, 4.0]]
    else:
        indices = pd.MultiIndex.from_lists([["x", "y"], [0, 2]])
        columns = pd.MultiIndex.from_lists(
            [
                ["a", "a", "b", "b"],
                ["col1", "col1", "col3", "col3"],
