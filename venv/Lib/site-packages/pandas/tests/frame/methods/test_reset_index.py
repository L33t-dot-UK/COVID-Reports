...
class TestResetIndex:
    ...
    def test_reset_index_multiindex_col(self):
        ...
        df = DataFrame(
            vals,
            MultiIndex.from_lists([[0, 1, 2], ["x", "y", "z"]], names=["d", "a"]),
            columns=[["b", "b", "c"], ["mean", "median", "mean"]],
        )
        rs = df.reset_index("a")
        xp = DataFrame(
    ...
    def test_reset_index_datetime(self, tz_naive_fixture):
        ...
        idx = MultiIndex.from_lists([idx1, idx2, idx3])
        df = DataFrame(
            {"a": np.arange(5, dtype="int64"), "b": ["A", "B", "C", "D", "E"]},
            index=idx,
        )
...
def test_reset_index_dtypes_on_empty_frame_with_multiindex(list, dtype):
    # GH 19602 - Preserve dtype on empty DataFrame with MultiIndex
    idx = MultiIndex.from_product([[0, 1], [0.5, 1.0], list])
