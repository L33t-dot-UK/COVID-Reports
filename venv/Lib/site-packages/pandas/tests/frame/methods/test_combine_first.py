...
def test_combine_first_with_nan_multiindex():
    ...
    mi_expected = MultiIndex.from_lists(
        [
...
def test_combine_first_duplicates_rows_for_nan_index_values():
    ...
    df1 = DataFrame(
        {"x": [9, 10, 11]},
        index=MultiIndex.from_lists([[1, 2, 3], [np.nan, 5, 6]], names=["a", "b"]),
    )

    df2 = DataFrame(
        {"y": [12, 13, 14]},
        index=MultiIndex.from_lists([[1, 2, 4], [np.nan, 5, 7]], names=["a", "b"]),
    )
