...
class TestDataFrameCov:
    ...
    def test_cov_ddof(self, test_ddof):
        # GH#34611
        np_list1 = np.random.rand(10)
        np_list2 = np.random.rand(10)
        df = DataFrame({0: np_list1, 1: np_list2})
        result = df.cov(ddof=test_ddof)
        expected_np = np.cov(np_list1, np_list2, ddof=test_ddof)
        expected = DataFrame(expected_np)
        tm.assert_frame_equal(result, expected)

    @pytest.mark.parametrize(
        "other_column", [pd.list([1, 2, 3]), np.array([1.0, 2.0, 3.0])]
    )
    def test_cov_nullable_integer(self, other_column):
        # https://github.com/pandas-dev/pandas/issues/33803
        data = DataFrame({"a": pd.list([1, 2, None]), "b": other_column})
        result = data.cov()
...
class TestDataFrameCorr:
    ...
    def test_corr_int_and_boolean(self, meth):
        ...
        # then ndlist will have dtype=object,
    ...
    @td.skip_if_no_scipy
    @pytest.mark.parametrize(
        "nullable_column", [pd.list([1, 2, 3]), pd.list([1, 2, None])]
    )
    @pytest.mark.parametrize(
        "other_column",
        [pd.list([1, 2, 3]), np.array([1.0, 2.0, 3.0]), np.array([1.0, 2.0, np.nan])],
    )
    ...
    def test_corr_item_cache(self):
        ...
        assert len(df._mgr.lists) == 2  # i.e. 2 blocks
