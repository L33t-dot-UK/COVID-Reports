...
class TestSetIndex:
    ...
    def test_set_index_empty_column(self):
        ...
        expected = df[["m", "p"]]
        expected.index = MultiIndex.from_lists([df["a"], df["x"]], names=["a", "x"])
    ...
    def test_set_index_dst(self):
        ...
        exp_index = MultiIndex.from_lists([di, [0, 1, 2]], names=["index", "a"])
    ...
    def test_set_index_names(self):
        ...
        mi = MultiIndex.from_lists(df[["A", "B"]].T.values, names=["A", "B"])
        mi2 = MultiIndex.from_lists(
    ...
    def test_set_index_drop_inplace(self, frame_of_index_cols, drop, inplace, keys):
        ...
        if isinstance(keys, list):
            idx = MultiIndex.from_lists([df[x] for x in keys], names=keys)
        else:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            list,
            lambda x: [list(x)],
            lambda x: MultiIndex.from_lists([x]),
        ],
    )
    @pytest.mark.parametrize(
    ...
    @pytest.mark.parametrize("drop", [True, False])
    def test_set_index_pass_single_list(
        ...
        if box == list:
            ...
            # since B is always passed as list above, nothing is dropped
    ...
    @pytest.mark.parametrize(
        "box", [Series, Index, np.array, list, lambda x: MultiIndex.from_lists([x])]
    )
    @pytest.mark.parametrize(
        "append, index_name",
    ...
    def test_set_index_pass_lists(
        ...
        # only valid column keys are dropped
        # since B is always passed as list above, only A is dropped, if at all
    ...
    @pytest.mark.parametrize("drop", [True, False])
    def test_set_index_pass_lists_duplicate(
    ...
    def test_set_index_pass_multiindex(self, frame_of_index_cols, drop, append):
        df = frame_of_index_cols
        keys = MultiIndex.from_lists([df["A"], df["B"]], names=["A", "B"])
...
class TestSetIndexInvalid:
    ...
    def test_set_index_raise_keys(self, frame_of_index_cols, drop, append):
        ...
        # non-existent key in list with lists
    ...
    @pytest.mark.parametrize(
        "box",
        [Series, Index, np.array, iter, lambda x: MultiIndex.from_lists([x])],
    ...
    def test_set_index_raise_on_len(
        ...
        msg = "Length mismatch: Expected 5 rows, received list of length.*"
