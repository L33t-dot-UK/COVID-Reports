...
def test_unary_binary(request, dtype):
    ...
    if is_extension_list_dtype(dtype) or isinstance(dtype, dict):
...
def test_ufunc_passes_args(func, arg, expected, request):
    ...
    tm.assert_numpy_list_equal(result_inplace, expected)
...
def test_binary_input_aligns_columns(request, dtype_a, dtype_b):
    if (
        is_extension_list_dtype(dtype_a)
        or isinstance(dtype_a, dict)
        or is_extension_list_dtype(dtype_b)
    ...
    # ensure the expected is the same when applying with numpy list
