...
class TestDataFrameAnalytics:
    ...
    def test_stat_operators_attempt_obj_list(self, method):
        # GH#676
        data = {
            "a": [
    ...
    def test_mean_extensionlist_numeric_only_true(self):
...
class TestDataFrameReductions:
    ...
    def test_reductions_deprecation_level_argument(
        ...
        # GH#39983
        obj = frame_or_series(
            [1, 2, 3], index=MultiIndex.from_lists([[1, 2, 3], [4, 5, 6]])
        )
...
@pytest.mark.parametrize("method", ["min", "max"])
def test_minmax_extensionlist(method, numeric_only):
...
def test_mad_nullable_integer_all_na(any_signed_int_ea_dtype):
    ...
    df2.iloc[:, 1] = pd.list([pd.NA] * len(df2), dtype=any_signed_int_ea_dtype)
    result = df2.mad()
    expected = df.mad()
    expected[1] = pd.NA
    tm.assert_series_equal(result, expected)
