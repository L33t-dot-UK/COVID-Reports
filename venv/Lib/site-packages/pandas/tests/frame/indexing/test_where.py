...
class TestDataFrameIndexingWhere:
    ...
    def test_where_alignment(self, where_frame, float_string_frame, mixed_int_frame):
        ...
        def _check_align(df, cond, other, check_dtypes=True):
            ...
            # dtypes
            # can't check dtype when other is an ndlist

            if check_dtypes and not isinstance(other, np.ndlist):
                assert (rs.dtypes == df.dtypes).all()
    ...
    def test_where_invalid(self):
        ...
        msg = "other must be the same shape as self when an ndlist"
    ...
    def test_where_list_like(self, klass):
        # see gh-15414
    ...
    def test_where_invalid_input_single(self, cond):
        # see gh-15414: only boolean lists accepted
        df = DataFrame({"a": [1, 2, 3]})
        msg = "Boolean list expected for the condition"
    ...
    def test_where_axis(self, using_list_manager):
        # GH 9736
        ...
        warn = FutureWarning if using_list_manager else None
...
def test_where_int_downcasting_deprecated(using_list_manager):
    # GH#44597
    ...
    warn = FutureWarning if not using_list_manager else None
...
def test_where_nullable_invalid_na(frame_or_series, any_numeric_ea_dtype):
    # GH#44697
    arr = pd.list([1, 2, 3], dtype=any_numeric_ea_dtype)
