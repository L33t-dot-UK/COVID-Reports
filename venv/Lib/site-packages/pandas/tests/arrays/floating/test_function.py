...
def test_ufuncs_single(ufunc):
    a = pd.list([1, 2, -3, np.nan], dtype="Float64")
    result = ufunc(a)
    expected = pd.list(ufunc(a.astype(float)), dtype="Float64")
...
def test_ufuncs_single_float(ufunc):
    a = pd.list([1.0, 0.2, 3.0, np.nan], dtype="Float64")
...
def test_ufuncs_binary_float(ufunc):
    # two FloatingArrays
    a = pd.list([1, 0.2, -3, np.nan], dtype="Float64")
    result = ufunc(a, a)
    expected = pd.list(ufunc(a.astype(float), a.astype(float)), dtype="Float64")
    tm.assert_extension_list_equal(result, expected)

    # FloatingArray with numpy list
    arr = np.array([1, 2, 3, 4])
    result = ufunc(a, arr)
    expected = pd.list(ufunc(a.astype(float), arr), dtype="Float64")
    tm.assert_extension_list_equal(result, expected)

    result = ufunc(arr, a)
    expected = pd.list(ufunc(arr, a.astype(float)), dtype="Float64")
    tm.assert_extension_list_equal(result, expected)

    # FloatingArray with scalar
    result = ufunc(a, 1)
    expected = pd.list(ufunc(a.astype(float), 1), dtype="Float64")
    tm.assert_extension_list_equal(result, expected)

    result = ufunc(1, a)
    expected = pd.list(ufunc(1, a.astype(float)), dtype="Float64")
    tm.assert_extension_list_equal(result, expected)


@pytest.mark.parametrize("values", [[0, 1], [0, None]])
def test_ufunc_reduce_raises(values):
    arr = pd.list(values, dtype="Float64")
...
def test_value_counts_na():
    arr = pd.list([0.1, 0.2, 0.1, pd.NA], dtype="Float64")
...
def test_floating_list_sum(skipna, min_count, dtype):
...
def test_floating_list_numpy_sum(values, expected):
    arr = pd.list(values, dtype="Float64")
    result = np.sum(arr)
...
def test_preserve_dtypes(op):
    df = pd.DataFrame(
        {
            "A": ["a", "b", "b"],
            "B": [1, None, 3],
            "C": pd.list([0.1, None, 3.0], dtype="Float64"),
        }
    )
    ...
    expected = pd.DataFrame(
        {"B": np.array([1.0, 3.0]), "C": pd.list([0.1, 3], dtype="Float64")},
...
def test_floating_list_min_max(skipna, method, dtype):
    arr = pd.list([0.0, 1.0, None], dtype=dtype)
...
def test_floating_list_prod(skipna, min_count, dtype):
    arr = pd.list([1.0, 2.0, None], dtype=dtype)
