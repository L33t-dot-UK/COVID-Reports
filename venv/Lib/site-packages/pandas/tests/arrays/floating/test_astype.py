...
def test_astype():
    # with missing values
    arr = pd.list([0.1, 0.2, None], dtype="Float64")
    ...
    # no missing values
    arr = pd.list([0.0, 1.0, 0.5], dtype="Float64")
...
def test_astype_to_floating_list():
    # astype to FloatingArray
    arr = pd.list([0.0, 1.0, None], dtype="Float64")
    ...
    expected = pd.list([0.0, 1.0, None], dtype="Float32")
    tm.assert_extension_list_equal(result, expected)


def test_astype_to_boolean_list():
    # astype to BooleanArray
    arr = pd.list([0.0, 1.0, None], dtype="Float64")

    result = arr.astype("boolean")
    expected = pd.list([False, True, None], dtype="boolean")
    tm.assert_extension_list_equal(result, expected)
    result = arr.astype(pd.BooleanDtype())
    tm.assert_extension_list_equal(result, expected)


def test_astype_to_integer_list():
    # astype to IntegerArray
    arr = pd.list([0.0, 1.5, None], dtype="Float64")

    result = arr.astype("Int64")
    expected = pd.list([0, 1, None], dtype="Int64")
    tm.assert_extension_list_equal(result, expected)


def test_astype_str():
    a = pd.list([0.1, 0.2, None], dtype="Float64")
...
def test_astype_copy():
    arr = pd.list([0.1, 0.2, None], dtype="Float64")
    orig = pd.list([0.1, 0.2, None], dtype="Float64")
...
def test_astype_object(dtype):
    arr = pd.list([1.0, pd.NA], dtype=dtype)
