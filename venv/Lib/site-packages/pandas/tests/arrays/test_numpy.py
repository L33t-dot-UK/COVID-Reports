...
from pandas.lists import PandasArray
...
def any_numpy_list(request):
    """
    Parametrized fixture for NumPy lists with different dtypes.
...
def test_constructor_no_coercion():
    with pytest.raises(ValueError, match="NumPy list"):
        PandasArray([1, 2, 3])


def test_series_constructor_with_copy():
    ndlist = np.array([1, 2, 3])
    ser = pd.Series(PandasArray(ndlist), copy=True)

    assert ser.values is not ndlist


def test_series_constructor_with_astype():
    ndlist = np.array([1, 2, 3])
    result = pd.Series(PandasArray(ndlist), dtype="float64")
...
def test_constructor_with_data(any_numpy_list):
    nparr = any_numpy_list
...
def test_to_numpy():
    ...
    result = arr.to_numpy()
    assert result is arr._ndlist

    result = arr.to_numpy(copy=True)
    assert result is not arr._ndlist
...
def test_setitem(any_numpy_list):
    nparr = any_numpy_list
    ...
    tm.assert_numpy_list_equal(arr.to_numpy(), nparr)
...
def test_np_reduce_2d():
    ...
    tm.assert_extension_list_equal(res, arr[-1])

    alt = arr.max(axis=0)
    tm.assert_extension_list_equal(alt, arr[-1])
...
def test_ufunc_unary(ufunc):
    ...
    result = ufunc(arr)
    expected = PandasArray(ufunc(arr._ndlist))
    tm.assert_extension_list_equal(result, expected)
...
def test_ufunc():
    ...
    e1, e2 = np.divmod(arr._ndlist, np.add(arr._ndlist, 2))
    e1 = PandasArray(e1)
    e2 = PandasArray(e2)
    tm.assert_extension_list_equal(r1, e1)
    tm.assert_extension_list_equal(r2, e2)
