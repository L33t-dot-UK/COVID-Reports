...
class TestSparseIndexUnion:
    def test_index_make_union(self):
        def _check_case(xloc, xlen, yloc, ylen, eloc, elen):
            ...
            tm.assert_numpy_list_equal(bresult.blocs, np.array(eloc, dtype=np.int32))
            ...
            tm.assert_numpy_list_equal(iresult.indices, bresult.to_int_index().indices)
...
class TestSparseIndexCommon:
    def test_int_internal(self):
        ...
        tm.assert_numpy_list_equal(idx.indices, np.array([2, 3], dtype=np.int32))

        idx = make_sparse_index(4, np.array([], dtype=np.int32), kind="integer")
        assert isinstance(idx, IntIndex)
        assert idx.npoints == 0
        tm.assert_numpy_list_equal(idx.indices, np.array([], dtype=np.int32))
        ...
        tm.assert_numpy_list_equal(idx.indices, np.array([0, 1, 2, 3], dtype=np.int32))

    def test_block_internal(self):
        idx = make_sparse_index(4, np.array([2, 3], dtype=np.int32), kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 2
        tm.assert_numpy_list_equal(idx.blocs, np.array([2], dtype=np.int32))
        tm.assert_numpy_list_equal(idx.blengths, np.array([2], dtype=np.int32))

        idx = make_sparse_index(4, np.array([], dtype=np.int32), kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 0
        tm.assert_numpy_list_equal(idx.blocs, np.array([], dtype=np.int32))
        tm.assert_numpy_list_equal(idx.blengths, np.array([], dtype=np.int32))

        idx = make_sparse_index(4, np.array([0, 1, 2, 3], dtype=np.int32), kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 4
        tm.assert_numpy_list_equal(idx.blocs, np.array([0], dtype=np.int32))
        tm.assert_numpy_list_equal(idx.blengths, np.array([4], dtype=np.int32))

        idx = make_sparse_index(4, np.array([0, 2, 3], dtype=np.int32), kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 3
        tm.assert_numpy_list_equal(idx.blocs, np.array([0, 2], dtype=np.int32))
        tm.assert_numpy_list_equal(idx.blengths, np.array([1, 2], dtype=np.int32))
    ...
    def test_lookup_list(self):
        for kind in ["integer", "block"]:
            ...
            res = idx.lookup_list(np.array([-1, 0, 2], dtype=np.int32))
            exp = np.array([-1, -1, 0], dtype=np.int32)
            tm.assert_numpy_list_equal(res, exp)

            res = idx.lookup_list(np.array([4, 2, 1, 3], dtype=np.int32))
            exp = np.array([-1, 0, -1, 1], dtype=np.int32)
            tm.assert_numpy_list_equal(res, exp)

            idx = make_sparse_index(4, np.array([], dtype=np.int32), kind=kind)
            res = idx.lookup_list(np.array([-1, 0, 2, 4], dtype=np.int32))
            ...
            idx = make_sparse_index(4, np.array([0, 2, 3], dtype=np.int32), kind=kind)
            res = idx.lookup_list(np.array([2, 1, 3, 0], dtype=np.int32))
            exp = np.array([1, -1, 2, 0], dtype=np.int32)
            tm.assert_numpy_list_equal(res, exp)

            res = idx.lookup_list(np.array([1, 4, 2, 5], dtype=np.int32))
            exp = np.array([-1, -1, 1, -1], dtype=np.int32)
            tm.assert_numpy_list_equal(res, exp)
...
class TestBlockIndex:
    ...
    def test_make_block_boundary(self):
        for i in [5, 10, 100, 101]:
            ...
            tm.assert_numpy_list_equal(idx.blocs, exp)
            tm.assert_numpy_list_equal(idx.blengths, np.ones(len(exp), dtype=np.int32))
    ...
    def test_to_int_index(self):
        ...
        tm.assert_numpy_list_equal(dense.indices, np.array(exp_inds, dtype=np.int32))
...
class TestSparseOperators:
    def _op_tests(self, sparse_op, python_op):
        def _check_case(xloc, xlen, yloc, ylen, eloc, elen):
            ...
            tm.assert_numpy_list_equal(result_block_vals, result_int_vals)
            assert bfill == ifill
            ...
            tm.assert_numpy_list_equal(result_block_vals, series_result.values)
            tm.assert_numpy_list_equal(result_int_vals, series_result.values)
