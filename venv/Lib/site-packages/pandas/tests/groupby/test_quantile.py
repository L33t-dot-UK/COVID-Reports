...
def test_quantile_list_no_sort():
...
def test_quantile_list_multiple_levels():
    df = DataFrame(
...
def test_groupby_quantile_with_listlike_q_and_int_columns(frame_size, groupby, q):
    # GH30289
...
@pytest.mark.parametrize(
    "values",
    [
        pd.list([1, 0, None] * 2, dtype="Int64"),
        pd.list([True, False, None] * 2, dtype="boolean"),
    ],
)
@pytest.mark.parametrize("q", [0.5, [0.0, 0.5, 1.0]])
def test_groupby_quantile_nullable_list(values, q):
