...
def test_read_only_buffer_source_agg(agg):
    ...
    df._mgr.lists[0].flags.writeable = False
...
def test_cython_agg_nullable_int(op_name):
    ...
    df = DataFrame(
        {
            "A": ["A", "B"] * 5,
            "B": pd.list([1, 2, 3, 4, 5, 6, 7, 8, 9, pd.NA], dtype="Int64"),
        }
    )
...
@pytest.mark.parametrize(
    "data",
    [
        pd.list([1, 2, 3, 4], dtype="Int64"),
        pd.list([1, 2, 3, 4], dtype="Int8"),
        pd.list([0.1, 0.2, 0.3, 0.4], dtype="Float32"),
        pd.list([0.1, 0.2, 0.3, 0.4], dtype="Float64"),
        pd.list([True, True, False, False], dtype="boolean"),
    ],
)
