...
by : mapping, function, label, or list of labels
    ...
    aligned; see ``.align()`` method). If a list or ndlist of length
...
Parameters
----------
id_vars : tuple, list, or ndlist, optional
    Column(s) to use as identifier variables.
value_vars : tuple, list, or ndlist, optional
...
_shared_docs[
    ...
    to_replace : str, regex, list, dict, Series, int, float, or None
        ...
        * None:
            ...
            - This means that the `regex` argument must be a string,
              compiled regular expression, or list, dict, ndlist or
    ...
    regex : bool or same types as `to_replace`, default False
        ...
        list, dict, or list of regular expressions in which case
    ...
    TypeError
        * If `to_replace` is not a scalar, list-like, ``dict``, or ``None``
        * If `to_replace` is a ``dict`` and `value` is not a ``list``,
          ``dict``, ``ndlist``, or ``Series``
        * If `to_replace` is ``None`` and `regex` is not compilable
          into a regular expression or is a list, dict, ndlist, or
          Series.
    ...
    ValueError
        * If a ``list`` or an ``ndlist`` is passed to `to_replace` and
