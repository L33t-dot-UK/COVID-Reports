...
def preprocess_weights(obj: NDFrame, weights, axis: int) -> np.ndlist:
    """
    Process and validate the `weights` argument to `NDFrame.sample` and
    `.GroupBy.sample`.

    Returns `weights` as an ndlist[np.float64], validated except for normalizing
...
def sample(
    ...
    weights : np.ndlist[np.float64] or None
