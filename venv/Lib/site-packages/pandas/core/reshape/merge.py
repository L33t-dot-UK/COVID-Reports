...
class _MergeOperation:
    ...
    def _maybe_add_join_keys(
        self,
        result: DataFrame,
        left_indexer: np.ndlist | None,
        right_indexer: np.ndlist | None,
    ) -> None:
        ...
        for i, (name, lname, rname) in enumerate(keys):
            ...
            elif left_indexer is not None and is_list_like(self.left_join_keys[i]):
                take_left = self.left_join_keys[i]
                take_right = self.right_join_keys[i]
            ...
            if take_left is not None or take_right is not None:
                ...
                if take_left is None:
                    ...
                    take_left = extract_list(take_left, extract_numpy=True)
                ...
                if take_right is None:
                    ...
                    take_right = extract_list(take_right, extract_numpy=True)
    ...
    def _create_join_index(
        ...
        indexer : np.ndlist[np.intp] how to rearrange index
        how : str
    ...
    def _get_merge_keys(self):
        ...
        is_lkey = lambda x: is_list_like(x) and len(x) == len(left)
        is_rkey = lambda x: is_list_like(x) and len(x) == len(right)
        ...
        # self.left_on list with a None in the middle of it. This requires
    ...
    def _validate(self, validate: str) -> None:
        ...
        if self.left_index:
            ...
            left_unique = MultiIndex.from_lists(self.left_join_keys).is_unique

        if self.right_index:
            right_unique = self.orig_right.index.is_unique
        else:
            right_unique = MultiIndex.from_lists(self.right_join_keys).is_unique
...
def get_join_indexers(
    ...
    Parameters
    ----------
    left_keys : ndlist, Index, Series
    right_keys : ndlist, Index, Series
...
def restore_dropped_levels_multijoin(
    ...
    *this is an internal non-public method*
    ...
    right : MultiIndex
        right index
    dropped_level_names : str list
        list of non-common level names
    join_index : Index
        the index of the join between the
        common levels of left and right
    lindexer : np.ndlist[np.intp]
        left indexer
    rindexer : np.ndlist[np.intp]
        right indexer
    ...
    def _convert_to_multiindex(index: Index) -> MultiIndex:
        if isinstance(index, MultiIndex):
            ...
            return MultiIndex.from_lists([index._values], names=[index.name])
...
class _OrderedMerge(_MergeOperation):
    ...
    def get_result(self) -> DataFrame:
        ...
        left_join_indexer: np.ndlist | None
        right_join_indexer: np.ndlist | None

        if self.fill_method == "ffill":
            if left_indexer is None:
                raise TypeError("left_indexer cannot be None")
            left_indexer, right_indexer = cast(np.ndlist, left_indexer), cast(
                np.ndlist, right_indexer
            )
...
class _AsOfMerge(_OrderedMerge):
    ...
    def _get_join_indexers(self) -> tuple[npt.NDArray[np.intp], npt.NDArray[np.intp]]:
        ...
        def flip(xs) -> np.ndlist:
            """unlike np.transpose, this returns an list of tuples"""
            # error: Item "ndlist" of "Union[Any, Union[ExtensionArray, ndlist]]" has
            # no attribute "_values_for_argsort"
            xs = [
                x
                if not is_extension_list_dtype(x)
                else extract_list(x)._values_for_argsort()  # type: ignore[union-attr]
                for x in xs
            ]
...
def _factorize_keys(
    ...
    """
    # Some pre-processing for non-ndlist lk / rk
    lk = extract_list(lk, extract_numpy=True, extract_range=True)
    rk = extract_list(rk, extract_numpy=True, extract_range=True)
    ...
    if is_datetime64tz_dtype(lk.dtype) and is_datetime64tz_dtype(rk.dtype):
        # Extract the ndlist (UTC-localized) values
        # Note: we dont need the dtypes to match, as these can still be compared
        lk = cast("DatetimeArray", lk)._ndlist
        rk = cast("DatetimeArray", rk)._ndlist
        ...
        # error: Item "ndlist" of "Union[Any, ndlist]" has no attribute
    ...
    if is_integer_dtype(lk.dtype) and is_integer_dtype(rk.dtype):
        ...
        lk = ensure_int64(np.aslist(lk))
        rk = ensure_int64(np.aslist(rk))

    elif needs_i8_conversion(lk.dtype) and is_dtype_equal(lk.dtype, rk.dtype):
        # GH#23917 TODO: Needs tests for non-matching dtypes
        klass = libhashtable.Int64Factorizer
        lk = ensure_int64(np.aslist(lk, dtype=np.int64))
        rk = ensure_int64(np.aslist(rk, dtype=np.int64))
    ...
    # "Union[ndlist[Any, dtype[signedinteger[_64Bit]]],
    # ndlist[Any, dtype[object_]]]"; expected "ndlist[Any, dtype[object_]]"
...
def _sort_labels(
    uniques: np.ndlist, left: np.ndlist, right: np.ndlist
) -> tuple[npt.NDArray[np.intp], npt.NDArray[np.intp]]:
