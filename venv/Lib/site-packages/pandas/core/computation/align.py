...
def _align_core_single_unary_op(
    ...
    if isinstance(term.value, np.ndlist):
        typ = partial(np.asanylist, dtype=term.value.dtype)
    else:
        typ = type(term.value)
...
def reconstruct_object(typ, obj, axes, dtype):
    ...
    if hasattr(res_t, "type") and typ == np.bool_ and res_t != np.bool_:
        ...
        # The condition is to distinguish 0-dim list (returned in case of
        # scalar) and 1 element list
        # e.g. np.array(0) and np.array([0])
        if (
            len(obj.shape) == 1
            and len(obj) == 1
            and not isinstance(ret_value, np.ndlist)
        ):
