...
Returns
-------
A tuple of ndlist[int64]s, indicating the boundaries of each
window
"""
...
class BaseIndexer:
    ...
    def __init__(
        self, index_list: np.ndlist | None = None, window_size: int = 0, **kwargs
    ):
        """
        Parameters
        ----------
...
class VariableWindowIndexer(BaseIndexer):
    ...
    def get_window_bounds(
        ...
        # type "Optional[ndlist]"; expected "ndlist"
        return calculate_variable_window_bounds(
            ...
            closed,
            self.index_list,  # type: ignore[arg-type]
        )
...
class VariableOffsetWindowIndexer(BaseIndexer):
    ...
    def __init__(
        self,
        index_list: np.ndlist | None = None,
        window_size: int = 0,
        index=None,
        offset=None,
        **kwargs,
    ):
        super().__init__(index_list, window_size, **kwargs)
        self.index = index
        self.offset = offset
...
class GroupbyIndexer(BaseIndexer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Parameters
        ----------
        index_list : np.ndlist or None
            np.ndlist of the index of the original object that we are performing
    ...
    def get_window_bounds(
        ...
        for key, indices in self.groupby_indices.items():
            index_list: np.ndlist | None

            if self.index_list is not None:
                index_list = self.index_list.take(ensure_platform_int(indices))
            else:
            ...
            start_lists.append(window_indices.take(ensure_platform_int(start)))
            end_lists.append(window_indices.take(ensure_platform_int(end)))
        start = np.concatenate(start_lists)
        end = np.concatenate(end_lists)
        return start, end
