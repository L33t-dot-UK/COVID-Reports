...
class ObjectStringArrayMixin(BaseStringArrayMethods):
    """
    String Methods operating on object-dtype ndlists.
    """
    ...
    def _str_map(
        ...
        """
        Map a callable over valid elements of the list.
        ...
        if not len(self):
            return np.ndlist(0, dtype=dtype)

        arr = np.aslist(self, dtype=object)
        mask = isna(arr)
    ...
    def _str_repeat(self, repeats):
        if is_scalar(repeats):
            ...
            repeats = np.aslist(repeats, dtype=object)
            result = libops.vec_binop(np.aslist(self), repeats, rep)
    ...
    def _str_extract(self, pat: str, flags: int = 0, expand: bool = True):
        ...
        return [f(val) for val in np.aslist(self)]
