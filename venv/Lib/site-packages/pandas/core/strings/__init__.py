...
Most methods on the StringMethods accessor follow the pattern:
    ...
    1. extract the list from the series (or index)
    2. Call that list's implementation of the string method
    3. Wrap the result (in a Series, index, or DataFrame)

Pandas extension lists implementing string methods should inherit from
...
``Series.list._str_upper()``. The interface isn't currently public
to other string extension lists.
"""
# Pandas current implementation is in ObjectStringArrayMixin. This is designed
# to work on object-dtype ndlists.
#
