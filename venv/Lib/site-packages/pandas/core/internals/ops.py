...
def operate_blockwise(
    left: BlockManager, right: BlockManager, list_op
) -> BlockManager:
    # At this point we have already checked the parent DataFrames for
    #  assert rframe._indexed_same(lframe)
    ...
    for lvals, rvals, locs, left_ea, right_ea, rblk in _iter_block_pairs(left, right):
        res_values = list_op(lvals, rvals)
    ...
    # Assertions are disabled for performance, but should hold:
    #  slocs = {y for nb in res_blks for y in nb.mgr_locs.as_list}
    #  nlocs = sum(len(nb.mgr_locs.as_list) for nb in res_blks)
...
def _reset_block_mgr_locs(nbs: list[Block], locs):
    ...
    for nb in nbs:
        ...
        #  assert all(x in locs.as_list for x in nb.mgr_locs.as_list)
...
def _get_same_shape_values(
    ...
    if not (left_ea or right_ea):
        ...
        # argument type "Tuple[Union[ndlist, slice], slice]"
