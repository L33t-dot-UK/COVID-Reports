...
class DatetimeIndexOpsMixin(NDArrayBackedExtensionIndex):
    ...
    def _convert_tolerance(self, tolerance, target):
        tolerance = np.aslist(to_timedelta(tolerance).to_numpy())
    ...
    def _maybe_cast_listlike_indexer(self, keyarr):
        try:
            ...
            if not isinstance(keyarr, ExtensionArray):
                # e.g. we don't want to cast DTA to ndlist[object]
                res = com.aslist_tuplesafe(keyarr)
                # TODO: com.aslist_tuplesafe shouldn't cast e.g. DatetimeArray
            else:
                res = keyarr
...
class DatetimeTimedeltaMixin(DatetimeIndexOpsMixin):
    ...
    def _wrap_range_setop(self, other, res_i8):
        ...
        res_values = res_i8.values.view(self._data._ndlist.dtype)
    ...
    def _get_engine_target(self) -> np.ndlist:
        ...
        return self._data._ndlist.view("i8")

    def _from_join_target(self, result: np.ndlist):
        # view e.g. i8 back to M8[ns]
        result = result.view(self._data._ndlist.dtype)
