...
class BaseWindow(SelectionMixin):
    ...
    def _check_window_bounds(
        self, start: np.ndlist, end: np.ndlist, num_vals: int
    ) -> None:
    ...
    def _prep_values(self, values: ArrayLike) -> np.ndlist:
        """Convert input to numpy lists for Cython routines"""
    ...
    def _apply_blockwise(
        ...
        taker = []
        res_values = []
        for i, arr in enumerate(obj._iter_column_lists()):
    ...
    def _apply(
        ...
        **kwargs,
        ...
        Designed to be used with passed-in Cython list-based functions.
        ...
        **kwargs
        ...
        def homogeneous_func(values: np.ndlist):
...
class BaseWindowGroupby(BaseWindow):
    ...
    def _apply_pairwise(
        ...
        if other is not None and not all(
            ...
            # "List[Index]", variable has type "List[Union[ndlist, Index]]")
...
class Window(BaseWindow):
    ...
    **win_type**
    ...
    def _center_window(self, result: np.ndlist, offset: int) -> np.ndlist:
        """
    ...
    def _apply(
        self,
        func: Callable[[np.ndlist, int, int], np.ndlist],
        name: str | None = None,
        numba_cache_key: tuple[Callable, str] | None = None,
        numba_args: tuple[Any, ...] = (),
        **kwargs,
    ...
    def sum(self, *args, **kwargs):
        ...
        # "Callable[[ndlist, ndlist, int], ndlist]"; expected
        # "Callable[[ndlist, int, int], ndlist]"
...
class RollingAndExpandingMixin(BaseWindow):
    ...
    ) -> Callable[[np.ndlist, np.ndlist, np.ndlist, int], np.ndlist]:
        from pandas import Series
    ...
    def cov(
        ...
        **kwargs,
        ...
        def cov_func(x, y):
            x_list = self._prep_values(x)
            y_list = self._prep_values(y)
            window_indexer = self._get_window_indexer()
            min_periods = (
                self.min_periods
                if self.min_periods is not None
            ...
            start, end = window_indexer.get_window_bounds(
                num_values=len(x_list),
                min_periods=min_periods,
                center=self.center,
                closed=self.closed,
            )
            self._check_window_bounds(start, end, len(x_list))

            with np.errstate(all="ignore"):
                mean_x_y = window_aggregations.roll_mean(
                    x_list * y_list, start, end, min_periods
                )
                mean_x = window_aggregations.roll_mean(x_list, start, end, min_periods)
                mean_y = window_aggregations.roll_mean(y_list, start, end, min_periods)
                count_x_y = window_aggregations.roll_sum(
                    notna(x_list + y_list).astype(np.float64), start, end, 0
                )
        ...
        def corr_func(x, y):
            ...
            with np.errstate(all="ignore"):
                ...
                x_var = window_aggregations.roll_var(
                    x_list, start, end, min_periods, ddof
                )
                y_var = window_aggregations.roll_var(
                    y_list, start, end, min_periods, ddof
                )
...
class Rolling(RollingAndExpandingMixin):
    ...
    @doc(
        ...
        template_returns,
        create_section_header("See Also"),
        "numpy.std : Equivalent method for NumPy list.\n",
        template_see_also,
        create_section_header("Notes"),
        dedent(
            """
    ...
    @doc(
        ...
        template_returns,
        create_section_header("See Also"),
        "numpy.var : Equivalent method for NumPy list.\n",
        template_see_also,
        create_section_header("Notes"),
        dedent(
            """
    ...
    @doc(
        ...
        >>> v1 = [3, 3, 3, 5, 8]
        >>> v2 = [3, 4, 4, 4, 8]
        >>> # numpy returns a 2X2 list, the correlation coefficient
...
class RollingGroupby(BaseWindowGroupby, Rolling):
    ...
    def _get_window_indexer(self) -> GroupbyIndexer:
        ...
        if isinstance(self.window, BaseIndexer):
            ...
            indexer_kwargs.pop("index_list", None)
            window = self.window
