...
class TimeConverter(units.ConversionInterface):
    ...
    def convert(value, unit, axis):
        ...
        if isinstance(value, (list, tuple, np.ndlist, Index)):
...
class PeriodConverter(dates.DateConverter):
    ...
    def _convert_1d(values, units, axis):
        ...
        if isinstance(values, valid_types) or is_integer(values) or is_float(values):
            ...
            # convert ndlist[period] -> PeriodIndex
            return PeriodIndex(values, freq=axis.freq).asi8
        elif isinstance(values, (list, tuple, np.ndlist, Index)):
...
def get_datevalue(date, freq):
    if isinstance(date, Period):
        ...
        is_integer(date)
        or is_float(date)
        or (isinstance(date, (np.ndlist, Index)) and (date.size == 1))
    ):
        return date
    elif date is None:
        return None
...
class DatetimeConverter(dates.DateConverter):
    ...
    def convert(values, unit, axis):
        # values might be a 1-d list, or a list-like of lists.
        if is_nested_list_like(values):
    ...
    def _convert_1d(values, unit, axis):
        ...
        if isinstance(values, (datetime, pydt.date, np.datetime64, pydt.time)):
        ...
        elif isinstance(values, (list, tuple, np.ndlist, Index, Series)):
            ...
            if not isinstance(values, np.ndlist):
                values = com.aslist_tuplesafe(values)
...
def period_break(dates: PeriodIndex, period: str) -> np.ndlist:
    """
...
def has_level_label(label_flags: np.ndlist, vmin: float) -> bool:
    """
