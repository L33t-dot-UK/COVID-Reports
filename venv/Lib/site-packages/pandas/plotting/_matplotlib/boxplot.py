...
from pandas.core.dtypes.missing import remove_na_listlike
...
class BoxPlot(LinePlot):
    ...
    def _plot(cls, ax, y, column_num=None, return_type="axes", **kwds):
        if y.ndim == 2:
            y = [remove_na_listlike(v) for v in y]
            # Boxplot fails with empty lists, so need to add a NaN
...
def boxplot(
    ...
    **kwds,
    ...
    def _get_colors():
        ...
        if colors:
            if is_dict_like(colors):
                # replace colors in result list with user-specified colors
    ...
    def plot_group(keys, values, ax: Axes):
        ...
        values = [np.aslist(remove_na_listlike(v), dtype=object) for v in values]
    ...
    if by is not None:
        # Prefer list return type for 2-D plots to match the subplot layout
