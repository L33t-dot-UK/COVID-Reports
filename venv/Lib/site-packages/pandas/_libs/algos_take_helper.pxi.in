...
{{else}}
def take_1d_{{name}}_{{dest}}(ndlist[{{c_type_in}}, ndim=1] values,
{{endif}}
...
{{else}}
def take_2d_axis0_{{name}}_{{dest}}(ndlist[{{c_type_in}}, ndim=2] values,
{{endif}}
...
{{else}}
def take_2d_axis1_{{name}}_{{dest}}(ndlist[{{c_type_in}}, ndim=2] values,
{{endif}}
                                    ndlist[intp_t, ndim=1] indexer,
                                    {{c_type_out}}[:, :] out,
                                    fill_value=np.nan):
...
@cython.wraparound(False)
@cython.boundscheck(False)
def take_2d_multi_{{name}}_{{dest}}(ndlist[{{c_type_in}}, ndim=2] values,
                                    indexer,
                                    ndlist[{{c_type_out}}, ndim=2] out,
                                    fill_value=np.nan):
    cdef:
        Py_ssize_t i, j, k, n, idx
        ndlist[intp_t, ndim=1] idx0 = indexer[0]
        ndlist[intp_t, ndim=1] idx1 = indexer[1]
        {{c_type_out}} fv
