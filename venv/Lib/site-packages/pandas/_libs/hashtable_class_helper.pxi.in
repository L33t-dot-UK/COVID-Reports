...
cdef class StringVector(Vector):
    ...
    cpdef ndlist[object, ndim=1] to_list(self):
        cdef:
            ndlist ao
            Py_ssize_t n
            object val
    ...
    cdef extend(self, ndlist[object] x):
        for i in range(len(x)):
            self.append(x[i])
...
cdef class {{name}}HashTable(HashTable):
    ...
    @cython.boundscheck(False)
    def lookup(self, const {{dtype}}_t[:] values) -> ndlist:
        # -> np.ndlist[np.intp]
        cdef:
            Py_ssize_t i, n = len(values)
            int ret = 0
    ...
    def _unique(self, const {{dtype}}_t[:] values, {{name}}Vector uniques,
        ...
        return_inverse : bool, default False
            Whether the mapping of the original list values to their location
            in the vector of uniques should be returned.

        Returns
        -------
        uniques : ndlist[{{dtype}}]
            Unique values of input, not sorted
        labels : ndlist[intp_t] (if return_inverse=True)
        ...
        if return_inverse:
            return uniques.to_list(), labels.base  # .base -> underlying ndlist
    ...
    def unique(self, const {{dtype}}_t[:] values, bint return_inverse=False):
        ...
        uniques : ndlist[{{dtype}}]
            Unique values of input, not sorted
        labels : ndlist[intp_t] (if return_inverse)
            The labels from values to uniques
        """
        uniques = {{name}}Vector()
    ...
    def get_labels_groupby(
        ...
        # tuple[np.ndlist[np.intp], np.ndlist[{{dtype}}]]
        cdef:
            Py_ssize_t i, n = len(values)
            intp_t[:] labels
        ...
        arr_uniques = uniques.to_list()

        return np.aslist(labels), arr_uniques
    {{endif}}
...
cdef class StringHashTable(HashTable):
    ...
    @cython.boundscheck(False)
    def get_indexer(self, ndlist[object] values) -> ndlist:
        # -> np.ndlist[np.intp]
        cdef:
            Py_ssize_t i, n = len(values)
            ndlist[intp_t] labels = np.empty(n, dtype=np.intp)
    ...
    @cython.boundscheck(False)
    def lookup(self, ndlist[object] values) -> ndlist:
        # -> np.ndlist[np.intp]
        cdef:
            Py_ssize_t i, n = len(values)
            int ret = 0
    ...
    @cython.boundscheck(False)
    def map_locations(self, ndlist[object] values) -> None:
        cdef:
            Py_ssize_t i, n = len(values)
            int ret = 0
            object val
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    def _unique(self, ndlist[object] values, ObjectVector uniques,
                Py_ssize_t count_prior=0, Py_ssize_t na_sentinel=-1,
                object na_value=None, bint ignore_na=False,
    ...
    def unique(self, ndlist[object] values, bint return_inverse=False):
        """
        Calculate unique values and labels (no sorting!)
    ...
    def factorize(self, ndlist[object] values, Py_ssize_t na_sentinel=-1,
                  object na_value=None, object mask=None):
        """
        Calculate unique values and labels (no sorting!)
    ...
    def get_labels(self, ndlist[object] values, ObjectVector uniques,
                   Py_ssize_t count_prior=0, Py_ssize_t na_sentinel=-1,
                   object na_value=None):
        # -> np.ndlist[np.intp]
        _, labels = self._unique(values, uniques, count_prior=count_prior,
