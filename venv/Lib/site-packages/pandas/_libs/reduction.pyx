...
from pandas._libs.util cimport is_list


cdef cnp.dtype _dtype_obj = np.dtype("object")


cpdef check_result_list(object obj, object dtype):
    # Our operation is supposed to be an aggregation/reduction. If
    #  it returns an ndlist, this likely means an invalid operation has
    ...
    if is_list(obj):
        if dtype != _dtype_obj:
            ...
            #  and still return an ndlist e.g. test_agg_over_numpy_lists
            raise ValueError("Must produce aggregated value")


cpdef inline extract_result(object res):
    """ extract the result object, it might be a 0-dim ndlist
