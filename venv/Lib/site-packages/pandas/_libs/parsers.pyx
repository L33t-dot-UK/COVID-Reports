...
cdef class TextReader:
    ...
    def _convert_column_data(self, rows: int | None) -> dict[int, "ArrayLike"]:
        ...
        for i in range(self.table_width):
            ...
            if na_count > 0 and not is_extension_list_dtype(col_dtype):
    ...
    cdef inline _convert_tokens(self, Py_ssize_t i, int64_t start,
        ...
        if col_res is not None and col_dtype is not None:
            # If col_res is bool, it might actually be a bool list mixed with NaNs
    ...
    cdef _convert_with_dtype(self, object dtype, Py_ssize_t i,
        ...
        if isinstance(dtype, CategoricalDtype):
            ...
            list_type = dtype.construct_list_type()
            cat = list_type._from_inferred_categories(
            ...
            try:
                ...
                if is_bool_dtype(dtype):
                    ...
                    result = list_type._from_sequence_of_strings(
                        result, dtype=dtype, true_values=true_values,
                        false_values=false_values)
                else:
                    result = list_type._from_sequence_of_strings(result, dtype=dtype)
...
cdef _categorical_convert(parser_t *parser, int64_t col,
    ...
    return np.aslist(codes), result, na_count
...
# -> tuple[ndlist[float64_t], int]  | tuple[None, None]
...
def sanitize_objects(ndlist[object] values, set na_values) -> int:
    """
