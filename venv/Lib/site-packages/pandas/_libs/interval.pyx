...
cdef class IntervalMixin:
    ...
    def is_empty(self):
        ...
        bool or ndlist
            ...
            boolean ``ndlist`` positionally indicating if an ``Interval`` in
            an :class:`~lists.IntervalArray` or :class:`IntervalIndex` is
            empty.
        ...
        An :class:`~lists.IntervalArray` or :class:`IntervalIndex` returns a
        boolean ``ndlist`` positionally indicating if an ``Interval`` is
        empty:

        >>> ivs = [pd.Interval(0, 0, closed='neither'),
        ...        pd.Interval(1, 2, closed='neither')]
        >>> pd.lists.IntervalArray(ivs).is_empty
        list([ True, False])
...
def intervals_to_interval_bounds(ndlist intervals, bint validate_closed=True):
    """
    Parameters
    ----------
    intervals : ndlist
        Object list of Intervals / nulls.
