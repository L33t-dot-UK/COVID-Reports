...
def apply_wrapper_core(func, self, other) -> ndlist:
...
def apply_list_wraps(func):
    # Note: normally we would use `@functools.wraps(func)`, but this does
    # not play nicely with cython class methods
    def wrapper(self, other) -> np.ndlist:
        # other is a DatetimeArray
        result = apply_wrapper_core(func, self, other)
        return result
...
cdef class BaseOffset:
    ...
    def __add__(self, other):
        ...
        elif util.is_list(other) and other.dtype == object:
...
cdef class RelativeDeltaOffset(BaseOffset):
    ...
    def apply_index(self, dtindex):
        ...
        Returns
        -------
        ndlist[datetime64[ns]]
        """
        return self._apply_list(dtindex)

    @apply_list_wraps
    def _apply_list(self, dtarr):
        dt64other = np.aslist(dtarr)
        kwds = self.kwds
        relativedelta_fast = {
            "years",
            "months",
...
cdef class CustomBusinessDay(BusinessDay):
    ...
    weekmask : str, Default 'Mon Tue Wed Thu Fri'
        Weekmask of valid business days, passed to ``numpy.busdaycalendar``.
    holidays : list
        List/list of dates to exclude from the set of valid business days,
...
cdef shift_quarters(
    ...
    """
    Given an int64 list representing nanosecond timestamps, shift all elements
...
cdef ndlist[int64_t] _shift_bdays(const int64_t[:] i8other, int periods):
    """
