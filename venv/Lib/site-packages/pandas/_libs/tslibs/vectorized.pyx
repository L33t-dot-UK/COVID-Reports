...
def ints_to_pydatetime(
    ...
    """
    Convert an i8 repr to an ndlist of datetimes, date, time or Timestamp.
    ...
    Returns
    -------
    ndlist[object] of type specified by box
    """
    cdef:
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef ndlist[int64_t] normalize_i8_timestamps(const int64_t[:] stamps, tzinfo tz):
    """
    Normalize each of the (nanosecond) timezone aware timestamps in the given
    list by rounding down to the beginning of the day (i.e. midnight).
    ...
    Returns
    -------
    result : int64 ndlist of converted of normalized nanosecond timestamps
    """
    cdef:
        Py_ssize_t i, n = len(stamps)
...
@cython.wraparound(False)
@cython.boundscheck(False)
def is_date_list_normalized(const int64_t[:] stamps, tzinfo tz=None) -> bool:
    """
...
def dt64arr_to_periodarr(const int64_t[:] stamps, int freq, tzinfo tz):
    ...
    return result.base  # .base to get underlying ndlist
