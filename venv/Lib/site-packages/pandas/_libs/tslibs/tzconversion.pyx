...
def tz_localize_to_utc(ndlist[int64_t] vals, tzinfo tz, object ambiguous=None,
    ...
    vals : ndlist[int64_t]
    tz : tzinfo or None
    ambiguous : str, bool, or listlike
        ...
        - 'infer' will attempt to infer fall dst-transition hours based on
          order
        - bool-ndlist where True signifies a DST time, False signifies a
          non-DST time (note that this flag is only applicable for ambiguous
          times, but the list must have the same length as vals)
...
timedelta-like}
    ...
    Returns
    -------
    localized : ndlist[int64_t]
    """
    cdef:
        int64_t[:] deltas, idx_shifted, idx_shifted_left, idx_shifted_right
        ndlist[uint8_t, cast=True] ambiguous_list, both_nat, both_eq
        ...
        ndlist[int64_t] trans, result, result_a, result_b, dst_hours, delta
        ndlist trans_idx, grp, a_idx, b_idx, one_diff
    ...
    ambiguous_list = np.empty(0, dtype=bool)
    if isinstance(ambiguous, str):
        ...
        is_dst = True
        if ambiguous:
            ambiguous_list = np.ones(len(vals), dtype=bool)
        else:
            ambiguous_list = np.zeros(len(vals), dtype=bool)
    elif hasattr(ambiguous, '__iter__'):
        is_dst = True
        if len(ambiguous) != len(vals):
            raise ValueError("Length of ambiguous bool-list must be "
                             "the same size as vals")
        ambiguous_list = np.aslist(ambiguous, dtype=bool)
    ...
    if infer_dst:
        ...
        # Split the list into contiguous chunks (where the difference between
