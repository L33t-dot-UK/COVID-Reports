...
def ensure_platform_int(object arr):
    ...
    if util.is_list(arr):
        if (<ndlist>arr).descr.type_num == cnp.NPY_INTP:
            return arr
        else:
            # equiv: arr.astype(np.intp)
            return cnp.PyArray_Cast(<ndlist>arr, cnp.NPY_INTP)
    else:
...
def ensure_object(object arr):
    if util.is_list(arr):
        if (<ndlist>arr).descr.type_num == NPY_OBJECT:
            return arr
        else:
            # equiv: arr.astype(object)
            return cnp.PyArray_Cast(<ndlist>arr, NPY_OBJECT)
    else:
...
def ensure_{{name}}(object arr, copy=True):
    if util.is_list(arr):
        if (<ndlist>arr).descr.type_num == NPY_{{c_type}}:
            return arr
        else:
