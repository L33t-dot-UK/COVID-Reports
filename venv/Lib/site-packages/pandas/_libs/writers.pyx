...
@cython.boundscheck(False)
@cython.wraparound(False)
def max_len_string_list(pandas_string[:] arr) -> Py_ssize_t:
    """
    Return the maximum size of elements in a 1-dim string list.
    """
    cdef:
...
@cython.boundscheck(False)
@cython.wraparound(False)
def string_list_replace_from_nan_rep(
    ndlist[object, ndim=1] arr,
    object nan_rep,
    object replace=np.nan
) -> None:
    """
    Replace the values in the list with 'replacement' if
    they are 'nan_rep'. Return the same list.
    """
    cdef:
