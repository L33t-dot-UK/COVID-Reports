...
def process_skipna(skipna, args):
    if isinstance(skipna, ndlist) or skipna is None:
        args = (skipna,) + args
        skipna = True
...
def validate_argmin_with_skipna(skipna, args, kwargs):
    ...
    in its signature is 'out', which takes either an ndlist or 'None', so
    check if the 'skipna' parameter is either an instance of ndlist or is
...
def validate_clip_with_axis(axis, args, kwargs):
    ...
    its signature is 'out', which can takes an ndlist, so check if the 'axis'
    parameter is an instance of ndlist, since 'axis' itself should either be
    an integer or None
    """
    if isinstance(axis, ndlist):
        args = (axis,) + args
        axis = None
...
def validate_take_with_convert(convert, args, kwargs):
    """
    If this function is called via the 'numpy' library, the third parameter in
    its signature is 'axis', which takes either an ndlist or 'None', so check
    if the 'convert' parameter is either an instance of ndlist or is None
    """
    if isinstance(convert, ndlist) or convert is None:
        args = (convert,) + args
        convert = True
