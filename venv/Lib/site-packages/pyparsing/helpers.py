...
def counted_list(
    ...
    *,
    ...
    The matched tokens returns the list of expr tokens as a list - the
    ...
    Example::
        ...
        counted_list(Word(alphas)).parse_string('2 ab cd ef')  # -> ['ab', 'cd']

        # in this parser, the leading integer value is given in binary,
        # '10' indicating that 2 values are in the list
        binary_constant = Word('01').set_parse_action(lambda t: int(t[0], 2))
        counted_list(Word(alphas), int_expr=binary_constant).parse_string('10 ab cd ef')  # -> ['ab', 'cd']

        # if other fields must be parsed after the count but before the
        # list items, give the fields results names and they will
        # be preserved in the returned ParseResults:
        count_with_metadata = integer + Word(alphas)("type")
        typed_list = counted_list(Word(alphanums), int_expr=count_with_metadata)("items")
        result = typed_list.parse_string("3 bool True True False")
        print(result.dump())
    ...
    def count_field_parse_action(s, l, t):
        nonlocal list_expr
        n = t[0]
        list_expr <<= (expr * n) if n else Empty()
    ...
    intExpr.set_name("listLen")
    intExpr.add_parse_action(count_field_parse_action, call_during_try=True)
    return (intExpr + list_expr).set_name("(len) " + str(expr) + "...")
