...
class PdfParser:
    ...
    def interpret_name(cls, raw, as_text=False):
        ...
        for m in cls.re_hashes_in_name.finditer(raw):
            if m.group(3):
                name += m.group(1) + bytelist.fromhex(m.group(3).decode("us-ascii"))
            else:
                name += m.group(1)
        if as_text:
    ...
    re_list_start = re.compile(whitespace_optional + br"\[")
    re_list_end = re.compile(whitespace_optional + br"]")
    ...
    def get_value(cls, data, offset, expect_indirect=None, max_nesting=-1):
        ...
        m = cls.re_list_start.match(data, offset)
        if m:
            offset = m.end()
            result = []
            m = cls.re_list_end.match(data, offset)
            while not m:
                value, offset = cls.get_value(data, offset, max_nesting=max_nesting - 1)
                result.append(value)
                if offset is None:
        ...
        if m:
            ...
            return bytelist.fromhex(hex_string.decode("us-ascii")), m.end()
