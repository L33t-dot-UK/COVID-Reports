...
def addEnglandToChart(df, colour, label, toDash):
    ...
    chart.addScatterplot(df["Dates"].tolist(), df["ENGLAND"].tolist(), colour, label, toDash, False)
...
def addRegionsToChart(df):
    df = df.reset_index() #remove dates as the index
    chart.addScatterplot(df["Dates"].tolist(), df["East of England"].tolist(), "red", "East of England", False, False)
    chart.addScatterplot(df["Dates"].tolist(), df["London"].tolist(), "blue", "London", False, False)
    chart.addScatterplot(df["Dates"].tolist(), df["Midlands"].tolist(), "black", "Midlands", False, False)
    chart.addScatterplot(df["Dates"].tolist(), df["North East and Yorkshire"].tolist(), "green", "North East and Yorkshire", False, False)
    chart.addScatterplot(df["Dates"].tolist(), df["North West"].tolist(), "indigo", "North West", False, False)
    chart.addScatterplot(df["Dates"].tolist(), df["South East"].tolist(), "orange", "South East", False, False)
    chart.addScatterplot(df["Dates"].tolist(), df["South West"].tolist(), "pink", "South West", False, False)
...
def drawBedsvsBeds():
    ...
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Total Beds Occupied")
    df2= hospitalData.joinTotalsDataSets("data/hospitalData", "Total Beds Occupied Covid")
    ...
    chart.addScatterplot(df2["Dates"].tolist(), bedCapArr, 'grey', 'Estimated Bed Capacity', True, False) 

    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Covid Absences")
    addEnglandToChart(df1, "indigo", "COVID19 NHS Staff Absences", True)

    chart.drawChart("Date", "Percentage of Beds Used / Total Beds Occupied (Scaled, in thousands)", "COVID-19: Number of Occupied Beds Used by COVID Patients (England)", "HOSDATA_PercentBedsUsed", True)
...
def drawMechBeds():
    ...
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "MV Beds Occupied")
    df2= hospitalData.joinTotalsDataSets("data/hospitalData", "MV Beds Occupied Covid-19")
    ...
    chart.addScatterplot(df2["Dates"].tolist(), bedCapArr, 'grey', 'Estimated Critcal Care Bed Capacity', True, False) 

    chart.drawChart("Date", "Percentage of MV Beds Used / Total MV Beds Occupied (Scaled, in Hundreds)", "COVID-19: Number of Occupied Mechanical Ventalation Beds Used by COVID Patients (England)", "HOSDATA_MechBeds", True)
...
def drawAdsDiagsPerCap():
    ...
    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 0-5")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 0-5")
    ...
    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 6-17")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 6-17")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    addEnglandToChart(calcPerMillion(1, df), "blue", "Age Group: 6 - 17", False)
    #addEnglandToChart(df, "blue", "0 - 5", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 18-64")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 18-64")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    addEnglandToChart(calcPerMillion(2, df), "orange", "Age Group: 18 - 64", False)
    #addEnglandToChart(df, "orange", "0 - 5", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 65-84")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 65-84")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    addEnglandToChart(calcPerMillion(3, df), "indigo", "Age Group: 65 - 84", False)
    #addEnglandToChart(df, "indigo", "0 - 5", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 85+")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 85+")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
...
def drawC19Abs():
    ...
    chart.addScatterplot(govData.getGOVdateSeries(), govData.getHospitalCases(), "indigo", "People in Hospital with COVID-19", False, False)

    #df = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses Total")
    #df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions Total")
    #df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    #addEnglandToChart(df, "indigo", "Daily Hospital Cases", True)

    chart.addScatterplot(govData.getGOVdateSeries(), govData.getNewCases(), "orange", "COVID-19 Cases", False, False)
...
def drawRecovery():
    ...
    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 0-5")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 0-5")
    df2 = hospitalData.joinTotalsDataSets("data/hospitalData", "Discharges 0-5")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    df1["ENGLAND"] = df["ENGLAND"] - df2["ENGLAND"]
    addEnglandToChart(df, "green", "Age Group: 0 - 5", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 6-17")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 6-17")
    df2 = hospitalData.joinTotalsDataSets("data/hospitalData", "Discharges 6-17")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    df1["ENGLAND"] = df["ENGLAND"] - df2["ENGLAND"]
    addEnglandToChart(df, "blue", "Age Group: 6 - 17", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 18-64")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 18-64")
    df2 = hospitalData.joinTotalsDataSets("data/hospitalData", "Discharges 18-64")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    df1["ENGLAND"] = df["ENGLAND"] - df2["ENGLAND"]
    addEnglandToChart(df, "orange", "Age Group: 18 - 64", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 65-84")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 65-84")
    df2 = hospitalData.joinTotalsDataSets("data/hospitalData", "Discharges 65-84")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    df1["ENGLAND"] = df["ENGLAND"] - df2["ENGLAND"]
    addEnglandToChart(df, "indigo", "Age Group: 65 - 84", True)

    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Diagnoses 85+")
    df1 = hospitalData.joinTotalsDataSets("data/hospitalData", "Admissions 85+")
    df2 = hospitalData.joinTotalsDataSets("data/hospitalData", "Discharges 85+")
    df["ENGLAND"] = df["ENGLAND"] + df1["ENGLAND"]
    df1["ENGLAND"] = df["ENGLAND"] - df2["ENGLAND"]
...
def drawFromNursing():
    ...
    chart.clearChart()
    chart.setChartParams(False, False, True, True)
    df = hospitalData.joinTotalsDataSets("data/hospitalData", "Total HospAdm From Care Nursing")
...
#
    ...
    chart.addScatterplot(subset["Dates"], subset["ENGLAND"], "black", "test 1", False, False)

    #subset1['Dates'] = subset1['Dates'].dt.strftime('%m-%d')
    #chart.addScatterplot(subset1["Dates"], subset1["ENGLAND"], "black", "test 1", False, True)

    chart.drawChart("Date", "Number of People", "COVID-19: TEST", "HOSDATA_TESTA", True)
